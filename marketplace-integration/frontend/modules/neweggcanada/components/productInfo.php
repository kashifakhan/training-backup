...
class productInfo extends Component
{
    ...
    public static function updateDetails($value=[],$sync=[],$merchant_id,$webhook = false)
    {
        try
        {
            ...
            $resultDetails = Data::sqlRecords("SELECT product_title,long_description,product_price FROM `newegg_can_product` WHERE product_id='".$product_id."' LIMIT 0,1", "one", "select");
            if (!$result)
            {
            ...
            if($product_type!=$result['product_type'] && isset($sync['sync-fields']['product_type']))
            {
                ...
                $query = 'SELECT id,category_id FROM `newegg_can_category_map` where merchant_id="'.$merchant_id.'" AND product_type="'.addslashes($value['product_type']).'" LIMIT 0,1';
                ...
                {
                    $query = 'INSERT INTO `newegg_can_category_map`(`merchant_id`,`product_type`) VALUES("'.$merchant_id.'","'.addslashes($value['product_type']).'")';
            ...
            if (is_array($variants))
            {
                foreach ($variants as $variant)
                {
                    ...
                    $walresult = Data::sqlRecords("SELECT * FROM `newegg_can_product_variants` WHERE option_id='".$option_id."' LIMIT 0,1", 'one', 'select');
                    ...
                    if($updateProductDetails)
                    {
                        ...
                        $query="UPDATE `newegg_can_product` SET ".$updateProductDetails." WHERE product_id=".$product_id;
                        //echo $query."<hr>";
                        Data::sqlRecords($query,null,'update');
                    }
                    ...
                    if($updateNeweggProductVariant)
                    {
                        ...
                        $query="UPDATE `newegg_can_product_variants` SET ".$updateNeweggProductVariant." WHERE option_id=".$option_id;
                        //echo $query."<hr>";
                        Data::sqlRecords($query,null,'update');
                    }
                }
