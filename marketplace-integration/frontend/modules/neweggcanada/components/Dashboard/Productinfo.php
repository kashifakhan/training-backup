    ...
    public static function getTotalProducts($merchant_id)
    {
        if(is_null(self::$_totalProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "select count(*) as `count` from (SELECT * from `newegg_can_product` where `merchant_id`=".$merchant_id." ) as `neweggproduct` LEFT JOIN (select * from `newegg_can_product_variants` where `merchant_id`=".$merchant_id." ) as `neweggvariantprod` ON `neweggproduct`.`product_id` = `neweggvariantprod`.`product_id` LIMIT 0,1";
    ...
    public static function getActivatedProducts($merchant_id)
    {
        if(is_null(self::$_publishedProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "SELECT count(*) as `count` FROM ((SELECT `sku` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=".$merchant_id." AND `newegg_can_product`.`upload_status`='".Data::PRODUCT_STATUS_ACTIVATED."') UNION (SELECT `option_sku` AS `sku` FROM `newegg_can_product_variants` INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=".$merchant_id." AND `newegg_can_product_variants`.`upload_status`='".Data::PRODUCT_STATUS_ACTIVATED."')) as `merged_data`";
    ...
    public static function getDeactivatedProducts($merchant_id)
    {
        if(is_null(self::$_unpublishedProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "SELECT count(*) as `count` FROM ((SELECT `sku` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=".$merchant_id." AND `newegg_can_product`.`upload_status`='".Data::PRODUCT_STATUS_DEACTIVATED."') UNION (SELECT `option_sku` AS `sku` FROM `newegg_can_product_variants` INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=".$merchant_id." AND `newegg_can_product_variants`.`upload_status`='".Data::PRODUCT_STATUS_DEACTIVATED."')) as `merged_data`";
    ...
    public static function getErrorProducts($merchant_id)
    {
        if(is_null(self::$_stageProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "SELECT count(*) as `count` FROM ((SELECT `sku` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=".$merchant_id." AND `newegg_can_product`.`upload_status`='".Data::PRODUCT_STATUS_UPLOADED_WITH_ERROR."') UNION (SELECT `option_sku` AS `sku` FROM `newegg_can_product_variants` INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=".$merchant_id." AND `newegg_can_product_variants`.`upload_status`='".Data::PRODUCT_STATUS_UPLOADED_WITH_ERROR."')) as `merged_data`";
    ...
    public static function getNotUploadedProducts($merchant_id)
    {
        if(is_null(self::$_notUploadedProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "SELECT count(*) as `count` FROM ((SELECT `sku` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=".$merchant_id." AND `newegg_can_product`.`upload_status`='".Data::PRODUCT_STATUS_NOT_UPLOADED."') UNION (SELECT `option_sku` AS `sku` FROM `newegg_can_product_variants` INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=".$merchant_id." AND `newegg_can_product_variants`.`upload_status`='".Data::PRODUCT_STATUS_NOT_UPLOADED."')) as `merged_data`";
    ...
    public static function getSubmittedProducts($merchant_id)
    {
        if(is_null(self::$_processingProducts))
        {
            ...
            if(is_numeric($merchant_id)) {
                ...
                $query = "SELECT count(*) as `count` FROM ((SELECT `sku` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=".$merchant_id." AND `newegg_can_product`.`upload_status`='".Data::PRODUCT_STATUS_SUBMITTED."') UNION (SELECT `option_sku` AS `sku` FROM `newegg_can_product_variants` INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=".$merchant_id." AND `newegg_can_product_variants`.`upload_status`='".Data::PRODUCT_STATUS_SUBMITTED."')) as `merged_data`";
