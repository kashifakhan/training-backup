...
class Orderdetails extends Component
{
    ...
    public function syncorders($path,$config=[])
    {
        ...
        $count = 0;
        $shopifyError = "";
        $model = Data::sqlRecords("SELECT shop_url,token FROM `newegg_shop_detail` where id='" . $merchant_id . "'", 'one');
                            ...
                            foreach ($result['ItemInfoList'] as $val) {
                                ...
                                if ($collection == "") {
                                    ...
                                    if ($collectionOption == "") {
                                        ...
                                        continue;
                                    } elseif ($collectionOption && $value['OrderQty'] > $collectionOption['option_qty']) {
...
//        $shop = Data::sqlRecords("SELECT `shop_url` FROM `newegg_shop_detail` WHERE `merchant_id`='".$merchant_id ."'",'one');
        ...
        $path = 'order/ship/webhook/'.$merchant_id.'/'.date('d-m-Y').'/'.time();
        ...
        if ($orderData) {
            ...
            Helper::createLog("---Requested Shipment Data From Shopify---".PHP_EOL.json_encode($data),$path,'a+');
            ...
            if ($orderData->order_status_description == "Unshipped") {
                ...
                try {
                    ...
                    if (isset($data['fulfillments']) && !empty($data['fulfillments'])) {
                        ...
                        Helper::createLog("---newegg order response---".PHP_EOL.$response. PHP_EOL,$path,'a+');
