...
use frontend\modules\neweggcanada\components\ShopifyApiException;
use Yii;
...
class Orderdetails extends Component
{
    ...
    public function createorders($data = [], $config = [])
    {
        ...
        $error_array = "";
        $return = [];
        $model = Data::sqlRecords("SELECT shop_url,token FROM `newegg_can_shop_detail` where merchant_id='" . $merchant_id . "'", 'one');
        ...
        if (!empty($model) && !empty($model['token'])) {
            try {
                ...
                $shopifyError = "";
//                $data = Data::sqlRecords("SELECT * FROM `newegg_can_order_detail` WHERE merchant_id='" . $merchant_id . "' AND order_status_description='Unshipped' AND shopify_order_id IS NULL ", 'all');
                $sc = new ShopifyClientHelper($shopname, $token, NEWEGGCANADA_APP_KEY, NEWEGGCANADA_APP_KEY);
                ...
                if (count($data) > 0) {
                    ...
                    if (count($result) > 0) {
                        ...
                        if (empty($error_array)) {
                            ...
                            $sku = implode(',', $sku_array);
                            $query = "SELECT `order_number` FROM `newegg_can_order_detail` WHERE `merchant_id` = '" . $merchant_id . "' AND `order_number` = '" . $result['OrderNumber'] . "'";
                            ...
                            if ($order_number) {
                                try {
                                    ...
                                    /*$emailData = Data::sqlRecords('SELECT * FROM `newegg_can_shop_detail` WHERE `merchant_id`="' . $merchant_id . '"', 'one');
    ...
    public static function cancelorder($order_number)
    {
        ...
        if ($order_number) {
            try {
                $data = Data::sqlRecords("SELECT `order_number`,`seller_id` FROM `newegg_can_order_detail` WHERE `merchant_id`='" . $merchant_id . "' AND `order_number`='" . $order_number . "'", 'one');
                $body = array(
                    "Action" => $action,
                    "Value" => $value
                );
                ...
                if (!isset($value['Code']) && isset($value['IsSuccess'])) {
                    ...
                    $query = "UPDATE `newegg_can_order_detail` SET `order_status_description`='" . addslashes($value['Result']['OrderStatus']) . "' WHERE `merchant_id`='" . $merchant_id . "' AND `order_number`='" . $value['Result']['OrderNumber'] . "' AND `seller_id`= '" . $data['seller_id'] . "'";
                    Data::sqlRecords($query, null, 'update');
    ...
    public function shiporder($id)
    {
        ...
        if ($id) {
            try {
                $model = Data::sqlRecords("SELECT * FROM `newegg_can_order_detail` WHERE `merchant_id`='" . $merchant_id . "' AND `id`='" . $id . "'", 'one');
                ...
                $sc = new ShopifyClientHelper($storeDetail['shop_url'], $storeDetail['token'], NEWEGGCANADA_APP_KEY, NEWEGGCANADA_APP_SECRET);
                ...
                if (!empty($shipmentDetails) && !isset($shipmentDetails['errors'])) {
                    ...
                    if (isset($shipmentDetails['fulfillments']) && !empty($shipmentDetails['fulfillments'])) {
                        ...
                        if (!empty($Unfulfilled_item)) {
                            ...
                            $query = "UPDATE `newegg_can_order_detail` SET `unfulfilled_array`='" . $unfulfilled_array . "' WHERE `merchant_id` = '" . $merchant_id . "' AND `seller_id` ='" . $model['seller_id'] . "' AND `order_number` = '" . $model['order_number'] . "'";
                        ...
                        if (!isset($data['Code']) && $data['IsSuccess']) {
                            if (!empty($data['PackageProcessingSummary'])) {
                                if (isset($data['PackageProcessingSummary']['SuccessCount']) && ($data['PackageProcessingSummary']['SuccessCount']) > 0) {
                                    ...
                                    try {
                                        $query = "UPDATE `newegg_can_order_detail` SET `order_status_description`='" . $OrderStatus . "' WHERE `merchant_id` = '" . $merchant_id . "' AND `seller_id` = '" . $model['seller_id'] . "' AND `order_number` = '" . $model['order_number'] . "'";
...
//            $message = implode(',',$return['message']);
            /*Data::sqlRecords("UPDATE `newegg_can_order_detail` SET `order_error` = '".$message."' WHERE `merchant_id` = '".$merchant_id."' AND `id`='".$id."' ","update");*/

            $query = "UPDATE `newegg_can_order_detail` SET `order_error`='" . $message . "' WHERE `merchant_id` = '" . $merchant_id . "' AND `id` ='" . $id . "'";
...
//        $shop = Data::sqlRecords("SELECT `shop_url` FROM `newegg_can_shop_detail` WHERE `merchant_id`='".$merchant_id ."'",'one');
                ...
                try {
                    ...
                    if (isset($data['fulfillments']) && !empty($data['fulfillments'])) {
                        ...
                        if (!empty($Unfulfilled_item)) {
                            ...
                            $query = "UPDATE `newegg_can_order_detail` SET `unfulfilled_array`='" . $unfulfilled_array . "' WHERE `merchant_id` = '" . $merchant_id . "' AND `seller_id` ='" . $seller_id . "' AND `order_number` = '" . $order_number . "'";
                        ...
                        if (!isset($data['Code']) && $data['IsSuccess']) {
                            if (!empty($data['PackageProcessingSummary'])) {
                                if (isset($data['PackageProcessingSummary']['SuccessCount']) && ($data['PackageProcessingSummary']['SuccessCount']) > 0) {
                                    $OrderStatus = $data['Result']['OrderStatus'];
                                    try {
                                        $query = "UPDATE `newegg_can_order_detail` SET `order_status_description`='" . $OrderStatus . "' WHERE `merchant_id` = '" . $merchant_id . "' AND `seller_id` = '" . $seller_id . "' AND `order_number` = '" . $order_number . "'";
    ...
    public function viewOrder($id)
    {
        ...
        if ($id) {
            try {
                $orderdata = Data::sqlRecords("SELECT `order_status_description`,`order_number` FROM `newegg_can_order_detail` WHERE `merchant_id`='" . $merchant_id . "' AND `id`='" . $id . "'", 'one');
                $type = 'OrderDetails';
                $return = '';
                $body = array(
                    "OperationType" => "GetOrderInfoRequest",
                ...
                if (!empty($data['ResponseBody']['OrderInfoList'])) {
                    if (isset($data['ResponseBody']['OrderInfoList'])) {
                        foreach ($data['ResponseBody']['OrderInfoList'] as $value) {
                            ...
                            $sku = implode(',', $sku_array);
                            $query = "SELECT `order_number` FROM `newegg_can_order_detail` WHERE `merchant_id` = '" . $merchant_id . "' AND `order_number` = '" . $value['OrderNumber'] . "'";
                ...
                /*$query = "SELECT `order_data` FROM `newegg_can_order_detail` WHERE `merchant_id`='" . $merchant_id . "' AND `id`='" . $id . "'";
    ...
    public function syncorders($config = [])
    {
                        ...
                        if (count($result) > 0) {
                            ...
                            if (!empty($itemArray) && (count($result['ItemInfoList']) == count($itemArray))) {
                                ...
                                $Orderarray['order'] = array(
                                    ...
                                    "note" => "Newegg-Canada-Integration(newegg.ca)",
                                    'tags' => "newegg.ca",
                                    "email" => $result['CustomerEmailAddress'],
                                ...
                                if (!array_key_exists('errors', $response)) {
                                    ...
                                    try {
                                        $query = "UPDATE `newegg_can_order_detail` SET  shopify_order_name='" . $response['name'] . "',shopify_order_id='" . $response['id'] . "',lines_items='" . addslashes($lineArray) . "'
                                                    where merchant_id='" . $merchant_id . "' AND seller_id='" . $result['SellerID'] . "' AND order_number='" . $result['OrderNumber'] . "'";
        ...
        if ($count > 0 && count($error_array) > 0) {
            ...
            $config_data = Data::sqlRecords("SELECT * FROM `newegg_can_config` WHERE `merchant_id`= '" . $merchant_id . "' AND `data`='cancel_order'", 'one');
            ...
            foreach ($error_array as $order_error) {
                ...
                if ($result && ($result['order_number'] == $order_error['order_number'])) {
                    ...
                    $sql = 'INSERT INTO `newegg_can_order_import_error`(`order_number`,`merchant_id`,`error_reason`,`newegg_item_number`,`created_at`)
                            VALUES("' . $order_error['order_number'] . '","' . $order_error['merchant_id'] . '","' . addslashes($reason) . '","' . $newegg_item_no . '","' . $order_error['created_at'] . '")';
