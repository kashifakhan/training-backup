                ...
                foreach ($products as $value) {
                    ...
                    if (count($variants) > 1) {
                        foreach ($variants as $value1) {
                            ...
                            $query = "";
                            $queryObj = "";
                            $query = 'SELECT category_id FROM `newegg_can_category_map` where merchant_id="' . $merchant_id . '" AND product_type="' . addslashes($product_type) . '"';
                            ...
                            if ($neweggmodelmap) {
                                //newegg new product
//                            $updateResult = "";
                                $query = 'UPDATE `newegg_can_product` SET newegg_category="' . $neweggmodelmap['category_id'] . '" where product_id="' . $product_id . '"';
                                $updateResult = $connection->createCommand($query)->execute();
                            } else {
                                //newegg category map
                                $queryObj = "";
                                if (!empty($product_type)) {
                                    $query = 'INSERT INTO `newegg_can_category_map`(`merchant_id`,`product_type`)VALUES("' . $merchant_id . '","' . $product_type . '")';
                                    $queryObj = $connection->createCommand($query)->execute();
                                }
                            }

                            $neweggresult = $connection->createCommand("SELECT `option_id` FROM `newegg_can_product_variants` WHERE option_id='" . $option_id . "'")->queryOne();

                            if (!$neweggresult) {
                                $sql = "INSERT INTO `newegg_can_product_variants`(
                                ...
                                $model = Data::sqlRecords("UPDATE `newegg_can_product_variants` SET option_id='" . $option_id . "' ,product_id ='" . $product_id . "' ,merchant_id ='" . $merchant_id . "'where option_id='" . $option_id . "'", 'all', 'update');
                            }
                        }
                    ...
                    $newresult = "";
                    $newresult = $connection->createCommand("SELECT `product_id` FROM `newegg_can_product` WHERE product_id='" . $product_id . "'")->queryOne();
                    if (!$newresult) {
                        $new_product_flag = true;
                        $sql = "INSERT INTO `newegg_can_product` (`product_id`,`merchant_id`,`upload_status`,`shopify_product_type`) VALUES ('" . $product_id . "','" . $merchant_id . "','Not Uploaded','" . addslashes($product_type) . "')";
                        $model = $connection->createCommand($sql)->execute();
                    } else {
                        $new_product_flag = true;
                        $model = Data::sqlRecords("UPDATE `newegg_can_product` SET product_id ='" . $product_id . "' ,merchant_id ='" . $merchant_id . "' ,shopify_product_type ='" . addslashes($product_type) . "' where product_id='" . $product_id . "'", 'all', 'update');
                    }
                ...
                if (is_array($variantData) && count($variantData) > 0) {
                    ...
                    foreach ($variantData as $key => $val) {
                        ...
                        if ($i == 0) {
                            ...
                            //save in `newegg_can_product` table
                            $neweggresult = Data::sqlRecords("SELECT `product_id` FROM `newegg_can_product` WHERE product_id='" . $product_id . "' LIMIT 0,1", 'one', 'select');
                            if (!$neweggresult) {
                                $sql = "INSERT INTO `newegg_can_product` (`product_id`,`merchant_id`,`upload_status`,`shopify_product_type`) VALUES ('" . $product_id . "','" . $merchant_id . "','" . Data::PRODUCT_STATUS_NOT_UPLOADED . "','" . addslashes($data['product_type']) . "')";
                        ...
                        if ($countVariants > 1) {
                            ...
                            $neweggresult = Data::sqlRecords("SELECT `option_id` FROM `newegg_can_product_variants` WHERE option_id='" . $key . "' LIMIT 0,1", "one", "select");
                            if (!$neweggresult) {
                                $sql = "INSERT INTO `newegg_can_product_variants`(
                    ...
                    if (isset($data['product_type']) && $data['product_type']) {
                        ...
                        $query = 'SELECT category_id FROM `newegg_can_category_map` where merchant_id="' . $merchant_id . '" AND product_type="' . addslashes($data['product_type']) . '" LIMIT 0,1';
                        ...
                        if ($neweggmodelmap) {
                            ...
                            //newegg category map
                            $queryObj = "";
                            $query = 'INSERT INTO `newegg_can_category_map`(`merchant_id`,`product_type`)VALUES("' . $merchant_id . '","' . addslashes($data['product_type']) . '")';
