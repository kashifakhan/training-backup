...
use frontend\modules\neweggcanada\components\product\VariantsProduct;
...
class ProductApi extends Component
{
    public static function createProductOnNewegg($ids, $merchant_id, $connection)
    {
        ...
        if (count($ids) > 0) {
            ...
            foreach ($ids as $id) {
                $query = "SELECT `ncm`.`category_path`,product_id,title,`ngp`.`merchant_id`,vendor,sku,type,`ngp`.`shopify_product_type`,`variant_id`,description,mpn,`ngp`.`long_description`,image,qty,price,weight,vendor,upc,upload_status,`ngp`.`newegg_data`,`ngp`.`manufacturer`,`ngp`.`mapped_value_data`,`ngp`.`short_description`,`ngp`.`newegg_attributes`,`ngp`.`spn` FROM `newegg_can_product` ngp INNER JOIN `jet_product` jet ON `jet`.`id`=`ngp`.`product_id` INNER JOIN `newegg_can_category_map` ncm ON `ncm`.`product_type`=`ngp`.`shopify_product_type` WHERE `ngp`.`product_id`='" . $id . "' and `ncm`.`merchant_id`='" . MERCHANT_ID . "' LIMIT 1 ";
                $productArray = Data::sqlRecords($query, "one", "select");

                if (empty($productArray)) {
                    $query1 = "SELECT `jet`.sku FROM `newegg_can_product` ngp INNER JOIN `jet_product` jet ON `jet`.`id`=`ngp`.`product_id`  WHERE `ngp`.`product_id`='" . $id . "' and `ngp`.`merchant_id`='" . MERCHANT_ID . "' LIMIT 1 ";
                ...
                if ($productArray['type'] == 'simple') {
                    ...
                    if (isset($validateResponse['error'])) {
                        ...
                        $query1 = 'UPDATE `newegg_can_product` SET upload_status ="' . Data::PRODUCT_STATUS_NOT_UPLOADED . '", error="" where product_id="' . $id . '"';
                    ...
                    foreach ($server_output['ResponseBody'] as $key => $value) {
                        foreach ($value as $key1 => $val1) {
                            ...
                            foreach ($uploadProductIds as $pid) {
                                $query1 = 'UPDATE `newegg_can_product` SET upload_status ="' . $val1['RequestStatus'] . '", error="" where product_id="' . $pid . '"';
                                Data::sqlRecords($query1, null, 'update');
    ...
    public static function validateProduct($product)
    {
        ...
        if ($product['type'] == "simple") {
            ...
            /*if (!$isexistAttr) {
                ...
                $productVarArray = [];
                /* print_r($product);*/
                $query = 'SELECT ngg.option_id,option_sku,option_image,option_qty,option_mpn,option_price,option_weight,option_unique_id,ngg.upload_status FROM `jet_product_variants` jet INNER JOIN `newegg_can_product_variants` ngg ON jet.option_id=ngg.option_id WHERE jet.product_id="' . $product['product_id'] . '"';
                $productVarArray = Data::sqlRecords($query, "all", "select");
    ...
    public static function checkAttributes($product)
    {
        if (isset($product['newegg_attributes']) && !empty($product['newegg_attributes'])) {
            ...
            $query = 'SELECT `attribute_map_data` from newegg_can_attribute_map Where `shopify_product_type`="' . $product['shopify_product_type'] . '" and `newegg_category_id`="' . $category_id[0] . '"';
    ...
    {
        $query = 'SELECT `newegg_option_attributes` from newegg_can_product_variants Where `product_id`="' . $product['product_id'] . '"';
        $productVarArray = Data::sqlRecords($query, "one", "select");
    ...
    */
    public static function getProductBasicInfo($productArray)
    {
        ...
        $data['SellingPrice'] = trim($productArray['price']);
    ...
    public static function getSubCategoryProperty($productArray)
    {
        ...
        if ($productArray['newegg_data']) {
            ...
            if ($productArray['newegg_attributes']) {
                ...
                $query = 'SELECT `attribute_map_data` from newegg_can_attribute_map Where merchant_id="' . $productArray['merchant_id'] . '" and `shopify_product_type` COLLATE latin1_general_cs LIKE "' . $productArray['shopify_product_type'] . '" and `newegg_category_id`="' . $category_id[0] . '"';
    ...
    */
    ...
    {
        $query = "SELECT `value` FROM  `newegg_can_config` WHERE `merchant_id`='" . MERCHANT_ID . "' AND `data`='product_description'";
   ...
   */
                ...
                if (isset($server_output['IsSuccess']) && $server_output['IsSuccess']) {
                    foreach ($server_output['ResponseBody'] as $key => $value) {
                        foreach ($value as $key1 => $val1) {
                            ...
                            Data::sqlRecords($query, null, 'insert');
                            foreach ($ids as $pid) {
                                $query1 = 'UPDATE `newegg_can_product` SET upload_status ="' . $val1['RequestStatus'] . '" where product_id="' . $pid . '"';
                                Data::sqlRecords($query1, null, 'update');
    ...
    public static function validateProductBarcode($barcode, $variant_id, $merchant_id = null)
    {
        ...
        $query = "SELECT `merged_data`.* FROM ((SELECT `variant_id` FROM `newegg_can_product` INNER JOIN `jet_product` ON `newegg_can_product`.`product_id`=`jet_product`.`id` WHERE `newegg_can_product`.`merchant_id`=" . $merchant_id . " AND `jet_product`.`type`='simple' AND `jet_product`.upc='{$barcode}') UNION (SELECT `newegg_can_product_variants`.`option_id` AS `variant_id` FROM `newegg_can_product_variants`  INNER JOIN `jet_product_variants` ON `newegg_can_product_variants`.`option_id`=`jet_product_variants`.`option_id` WHERE `newegg_can_product_variants`.`merchant_id`=" . $merchant_id . " AND `jet_product_variants`.option_unique_id='{$barcode}')) as `merged_data`";
