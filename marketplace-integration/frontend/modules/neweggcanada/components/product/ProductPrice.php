...
{
    public static function updatePriceOnNewegg($products,$merchant_id = false,$cron = false)
    {  
        ...
        if (count($products) > 0 ){
            ...
            foreach ($products as $product) {
                ...
                if($first==$last){ 
                    self::getPrepareData($productArray,$item,$itemFeed,$newegg_envelope,$productUpload);
                }
                else{
                    if($cron){
    ...
    */
     ...
     *product basic info prepare
     */
    public static function getPrepareData($productArray,&$item,&$itemFeed,&$newegg_envelope,&$productUpload)
    {
        if ($productArray['type'] == 'simple') {
            ...
            $query = 'SELECT jet.option_id,option_title,option_sku,ngg.newegg_option_attributes,option_image,option_qty,option_price,option_weight,option_unique_id FROM `newegg_can_product_variants` ngg INNER JOIN `jet_product_variants` jet ON jet.option_id=ngg.option_id WHERE ngg.product_id="' . $productArray['id'] . '"';
