...
class ProductStatus extends Component
{
    ...
    public static function updateProductStatusResult($product)
    {
        foreach ($product as $productkey => $productvalue) {
                                ...
                                if ($csvArrayvalue[10]) {
                                    ...
                                    if ($selectData) {
                                        if ($selectData['type'] == 'simple') {
                                            $updateQuery = "UPDATE `newegg_can_product` SET `upload_status`='" . Data::PRODUCT_STATUS_ACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND product_id='" . $selectData['id'] . "'";
                                            Data::sqlRecords($updateQuery, null, "update");
                                        } else {
                                            $updateQuery = "UPDATE `newegg_can_product` SET `upload_status`='" . Data::PRODUCT_STATUS_ACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND product_id='" . $selectData['id'] . "'";
                                            Data::sqlRecords($updateQuery, null, "update");
                                            $updateVariantsQuery = "UPDATE `newegg_can_product_variants` SET `upload_status`='" . Data::PRODUCT_STATUS_ACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND option_id='" . $selectData['variant_id'] . "'";
                                        ...
                                        $selectData = Data::sqlRecords($selectQuery, 'one', 'select');
                                        if ($selectData) {
                                            $updateVariantsQuery = "UPDATE `newegg_can_product_variants` SET `upload_status`='" . Data::PRODUCT_STATUS_ACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND option_id='" . $selectData['option_id'] . "'";
                                            Data::sqlRecords($updateVariantsQuery, null, "update");
                                        }
                                    }
                                } else {
                                    ...
                                    if ($selectData) {
                                        if ($selectData['type'] == 'simple') {
                                            $updateQuery = "UPDATE `newegg_can_product` SET `upload_status`='" . Data::PRODUCT_STATUS_DEACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND product_id='" . $selectData['id'] . "'";
                                            Data::sqlRecords($updateQuery, null, "update");
                                        } else {
                                            $updateQuery = "UPDATE `newegg_can_product` SET `upload_status`='" . Data::PRODUCT_STATUS_DEACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND product_id='" . $selectData['id'] . "'";
                                            Data::sqlRecords($updateQuery, null, "update");
                                            $updateVariantsQuery = "UPDATE `newegg_can_product_variants` SET `upload_status`='" . Data::PRODUCT_STATUS_DEACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND option_id='" . $selectData['variant_id'] . "'";
                                            Data::sqlRecords($updateVariantsQuery, null, "update");
                                        }
                                    } else {
                                        $selectQuery = "SELECT `option_id` FROM jet_product_variants WHERE merchant_id='" . $merchantId . "' AND option_sku='" . $csvArrayvalue[0] . "'";
                                        $selectData = Data::sqlRecords($selectQuery, 'one', 'select');
                                        if ($selectData) {
                                            $updateVariantsQuery = "UPDATE `newegg_can_product_variants` SET `upload_status`='" . Data::PRODUCT_STATUS_DEACTIVATED . "' WHERE merchant_id='" . $merchantId . "' AND option_id='" . $selectData['option_id'] . "'";
                                            Data::sqlRecords($updateVariantsQuery, null, "update");
                                        }
                                    }
                                }
            ...
            $delete = "DELETE FROM `newegg_can_product_feed` WHERE `merchant_id`='" . $merchantId . "' AND `feed_id`='" . $productvalue['feed_id'] . "'";
