  ...
  /*
    ...
    */
    public static function getCategoryaArribute($val)
    {
        $array = [];
        $valData = explode(',',$val);
        $data=Data::sqlRecords("SELECT `attribute_map` FROM `newegg_can_attribute_map` WHERE `merchant_id`='".MERCHANT_ID."' AND `shopify_product_type`='".$valData[0]."'","one","select");
    ...
    {
        $array = [];
        $data=Data::sqlRecords("SELECT `attribute_map` FROM `newegg_can_attribute_map` WHERE `merchant_id`='".MERCHANT_ID."' AND `shopify_product_type`='".$val."'","one","select");
        $data = json_decode($data['attribute_map'],true);
        foreach ($data as $key => $value) {
            $shopify_attribute = explode('->', $value);
        ...
        public static function getAttributeValue($val)
    {
        $array = [];
        $valData = explode(',',$val);
        $categoryQuery = "SELECT category_path FROM `newegg_can_category_map` WHERE product_type='".$valData[0]."' AND category_id='".$valData[1]."' AND merchant_id='".MERCHANT_ID."'";
        ...
        $query = "SELECT `value` FROM `newegg_can_value_attribute_mapping` WHERE `merchant_id`='".MERCHANT_ID."' AND product_type='".$valData[0]."' AND category_id='".$valData[1]."' AND attribute_name='".$valData[3]."'";
    ...
    {
        $data=Data::sqlRecords("SELECT `mapped_value_data` FROM `newegg_can_value_mapping` WHERE `shopify_product_type`='".$val."' AND `merchant_id`='".MERCHANT_ID."'","one","select");
