...
class TestController extends NeweggMainController
{
    ...
    */
    public function actionTest()
    {
        ...
        $query = "UPDATE `newegg_can_product` ngg JOIN `jet_product` jp ON `ngg`.`product_id`=`jp`.`id` SET `ngg`.`spn`='beauty_ring',`ngg`.`upload_status`='".Data::PRODUCT_STATUS_ACTIVATED."' WHERE `jp`.`sku`='beauty_ring' AND `ngg`.`merchant_id`='".MERCHANT_ID."' AND `ngg`.`spn`='beauty_ring' AND `ngg`.`upload_status`='".Data::PRODUCT_STATUS_ACTIVATED."'";
               ...
               foreach ($csvArray as $csvArraykey => $csvArrayvalue) {
                                if($csvArraykey=0){
                                    ...
                                    if(isset($csvArrayvalue[0]) && !empty($csvArrayvalue[0])){
                                        if($csvArrayvalue[10]){
                                            $query = "UPDATE `newegg_can_product` ngg JOIN `jet_product` jp ON `ngg`.`product_id`=`jp`.`id` SET `ngg`.`spn`='" .$csvArrayvalue[0]."',`ngg`.`upload_status`='".Data::PRODUCT_STATUS_ACTIVATED."' WHERE `jp`.`sku`='".$csvArrayvalue[0]."' AND `ngg`.`merchant_id`='".MERCHANT_ID."'";
                                            Data::sqlRecords($query, null, "update");
                                        }
                                        else{
                                            $query = "UPDATE `newegg_can_product` ngg JOIN `jet_product` jp ON `ngg`.`product_id`=`jp`.`id` SET `ngg`.`spn`='" .$csvArrayvalue[0]."',`ngg`.`upload_status`='".Data::PRODUCT_STATUS_DEACTIVATED."' WHERE `jp`.`sku`='".$csvArrayvalue[0]."' AND `ngg`.`merchant_id`='".MERCHANT_ID."'";
                                            Data::sqlRecords($query, null, "update");
                                        }
                                    }
                                }
                ...
                if(isset($getResponse->NeweggEnvelope->Message->ProcessingReport->ProcessingSummary) && $getResponse->NeweggEnvelope->Message->ProcessingReport->ProcessingSummary->WithErrorCount>0){
                    if(isset($getResponse->NeweggEnvelope->Message->ProcessingReport->Result) && isset($getResponse->NeweggEnvelope->Message->ProcessingReport->Result->ErrorList)){
                        ...
                        if($checkProduct=='Error'){
                            ...
                            $query="update `newegg_can_product` set status='".Data::PRODUCT_STATUS_NOT_UPLOADED."' , error='".$errorFromNewegg."' where product_id='".$id."'";
                            Data::sqlRecords($query,null,"update");
                        }
                        else{
                            unset($value['ErrorList']['ErrorDescription'][0]);
                            ...
                            $errorFromNewegg = implode(',', $content);
                            $query="update `newegg_can_product` set status='".Data::PRODUCT_STATUS_UPLOADED_WITH_ERROR."' , error='".$errorFromNewegg."' where product_id='".$id."'";
                            Data::sqlRecords($query,null,"update");
                            $count++;
                        }
                    ...
                    foreach ($ids as $id) {
                        $query="update `newegg_can_product` set status='".Data::PRODUCT_STATUS_ACTIVATED."' where product_id='".$id."'";
                        Data::sqlRecords($query,null,"update");
                        $count++;
                    }
                }
...
';
    ...
    public function actionSqlquery(){
        $sql=Data::sqlRecords('SELECT * FROM `newegg_can_product` WHERE `merchant_id`="'.MERCHANT_ID.'"',"all","select");
        print_r($sql);die;
    }
                ...
                foreach ($userDetails as $value) 
                {
                    ...
                    if($sc != '')
                    {
                        ...
                        if(!isset($webhooks['errors']))
                        {
                            ...
                            {
                                if($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/neweggproductupdate'){
                                ...
                                elseif($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/neweggproductdelete'){
                                    $change_webhook_data['webhook']=[
                                            'id'=>$val['id'],
                                 ...
                                 elseif($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/isinstall'){
                                    $change_webhook_data['webhook']=[
                                            'id'=>$val['id'],
                                 ...
                                 elseif($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/createshipment'){
                                    $change_webhook_data['webhook']=[
                                            'id'=>$val['id'],
                                 ...
                                 elseif($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/cancelled'){
                                    $change_webhook_data['webhook']=[
                                            'id'=>$val['id'],
                                            'address'=>'https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/cancelled'
                                        ];
                                    $response=$sc->call("PUT","/admin/webhooks/".$val['id'].".json",$change_webhook_data);
                                }
                                 elseif($val['address']=='https://shopify.cedcommerce.com/integration/neweggcanada/shopifywebhook/neweggproductcreate'){
                                    $change_webhook_data['webhook']=[
                                            'id'=>$val['id'],
