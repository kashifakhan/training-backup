...
use frontend\modules\neweggcanada\models\NeweggManufacturerSearch;
use frontend\modules\neweggcanada\models\NeweggManufacturer;
...
class ManufacturerController extends NeweggMainController
{
    ...
    */
    ...
    {
        $manufacturerCollection=Data::sqlRecords("SELECT `manufacturer_name` FROM `newegg_can_manufacturer` WHERE merchant_id=".MERCHANT_ID." and status='".Data::MANUFACTURER_PENDING_STATUS."'","all","select");
        foreach ($manufacturerCollection as $key => $value) {
            ...
            if(isset($server_output['ResponseBody']['ManufacturerList']) && !empty($server_output['ResponseBody']['ManufacturerList'])){
                $query = 'UPDATE `newegg_can_manufacturer` SET  status="'.Data::MANUFACTURER_COMPLETE_STATUS.'" where merchant_id="'.MERCHANT_ID.'" ';
            ...
            if(isset($server_output['IsSuccess']) && !empty($server_output['IsSuccess'])){
                if($server_output['IsSuccess']==1){
                    try{
                        ...
                        $manufacturerCollection=Data::sqlRecords("SELECT `manufacturer_name` FROM `newegg_can_manufacturer` WHERE merchant_id=".MERCHANT_ID." and manufacturer_name='".$mn."'","one","select");
                        if(empty($manufacturerCollection)){
                            $model = Data::sqlRecords("INSERT INTO `newegg_can_manufacturer` (`merchant_id`,`manufacturer_name`,`manufacturer_url`,`manufacturer_support_email`,`manufacturer_support_phone`,`manufacturer_support_url`,`status`) VALUES ('".MERCHANT_ID."','".$mn."','".$mu."','".$mse."','".$msp."' , '".$msu."' , '".$status."')", 'all','insert');
                                ...
                                $query = 'UPDATE `newegg_can_manufacturer` SET  manufacturer_name="'.$mn.'",manufacturer_url="'.$mu.'",manufacturer_support_email="'.$mse.'",manufacturer_support_phone="'.$msp.'",manufacturer_support_url="'.$msu.'","status="'.$status.'" where merchant_id="'.MERCHANT_ID.'" ';
                                Data::sqlRecords($query,null,'update');
                                $msg['success']="Manufacturer Successfully Created";
                                $data = json_encode($msg);
                                echo $data;
