...
class VariantsvaluemapController extends NeweggMainController
{
    public function actionIndex($id)
    {
        if (Yii::$app->user->isGuest) {
            ...
            foreach ($productArray as $key => $value) {
                ...
                $query = "SELECT `mapped_value_data` FROM `newegg_can_product` WHERE merchant_id =" . MERCHANT_ID . " AND shopify_product_type='" . $value['product_type'] . "' AND  newegg_category = '" . $attributeValue['category']['id'] . "'";
                ...
                if (isset($name) && !empty($name) && !empty($mapper)) {
                    ...
                    foreach ($name as $key2 => $value2) {
                        ...
                        if (isset($mapper[$value2])) {
                            ...
                            $groupData = implode(',', $mapValue);
                            $value[$value2] = $groupData;
                            $i++;
                        }
                    }
                }
            ...
            return $this->render('index', ['data' => $productData, 'id' => $id]);
        }
    }
    ...
    public function actionSave()
    {
        ...
        if (!empty($data)) {
            $attributeMap = [];
            $delete = "DELETE FROM `newegg_can_variants_product_value_map` WHERE `merchant_id`='" . $merchant_id . "'AND `product_id`='" . $data['id'] . "'";
            ...
            if (isset($data['newegg']) && !empty($data['newegg'])) {
                foreach ($data['newegg'] as $items => $values) {
                    ...
                    foreach ($values as $key1 => $val1) {
                        ...
                        /* $query = "INSERT INTO `newegg_can_variants_product_value_map`(`merchant_id`, `shopify_product_type`,`category_id`,`product_id`,`mapped_value_data`) VALUES ('" . $merchant_id . "','" . addslashes($productType) . "','" . $categoryId . "','" . $data['id'] . "','" .addslashes($attributeMapData). "') ";
                         Data::sqlRecords($query, null, 'insert');*/
                        if (!empty($attributeMap)) {
                            $sql = 'UPDATE `newegg_can_product` SET  mapped_value_data="' . addslashes($attributeMapData) . '" where merchant_id="' . $merchant_id . '" and shopify_product_type="' . addslashes($productType) . '" and product_id="' . $data['id'] . '"';
                ...
                Yii::$app->session->setFlash('success', "No attribute value for mapping!!");
