        ...
        if($data && isset($data['type']))
        {
            foreach($data['type'] as $key=>$value)
            {
                ...
                if(is_array($value) && count($value)>0 && $value[0]!=""){
                    ...
                    $sql='UPDATE `newegg_can_category_map` SET  category_id="'.trim($category_id).'",category_path="'.trim($category_path).'",manufacturer="'.$manufacturer.'" where merchant_id="'.$merchant_id.'" and product_type="'.$key.'"';
                    ...
                    $model="";
                    $sql='UPDATE `newegg_can_category_map` SET  category_id="",category_path="" ,manufacturer="'.$manufacturer.'" where merchant_id="'.$merchant_id.'" and product_type="'.$key.'"';
                    $model = $connection->createCommand($sql)->execute();
                    $product="";
