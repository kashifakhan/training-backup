...
class GetnotificationController extends Controller 
{
	...
	public static function getNotification($merchant_id)
	{
        ...
        $walmartnotification = Data::sqlRecords("SELECT * FROM `common_notification` WHERE `enable`='yes' AND `marketplace`='newgg_canada' AND `to_date` >= '".$date."' AND `from_date` <='".$date."' ORDER BY `sort_order` ASC", 'all');
        $date = strtotime($date);
    ...
    public function actionSetread(){
        $date = date('Y/m/d', time());
        $walmartnotification = Data::sqlRecords("SELECT * FROM `common_notification` WHERE `enable`='yes' AND `marketplace`='newgg_canada' AND `to_date` >= '".$date."' ORDER BY `sort_order` ASC", 'all');
        foreach ($walmartnotification as $key => $value) {
