...
class NeweggWebhookController extends Controller
{
        ...
        if (is_array($data) && count($data) > 0) {
            ...
            if (isset($data['archiveSku']) && $data['archiveSku']) {
                foreach ($data['archiveSku'] as $key => $value) {
                    $message = ProductPrice::updatePriceOnNewegg($data, $merchant_id, true);
                }
            } else {
    ...
    public function actionIsinstall()
    {
        ...
        $path = \Yii::getAlias('@webroot') . '/var/neweggcanada/uninstall/' . date('d-m-Y') . '/' . $data['shopName'];
        if (!file_exists($path)) {
        ...
        $url = Yii::getAlias('@webneweggcanadaurl') . "/newegg-webhook/curlprocessforuninstall";
        $ch = curl_init();
