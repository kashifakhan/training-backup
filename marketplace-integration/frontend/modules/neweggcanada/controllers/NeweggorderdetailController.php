...
use frontend\modules\neweggcanada\models\NeweggCourtesyrefundDetailSearch;
use Yii;
...
class NeweggorderdetailController extends NeweggMainController
{
        ...
        if ($cron) {
            ...
            foreach ($data as $value) {
                if ($value['status'] == 'success' && isset($value['updated'])) {
                    ...
                    $errormsg = "There is error for some orders.Please <a href='https://shopify.cedcommerce.com/integration/neweggcanada/neweggorderimporterror/index'>click</a> to check failed order errors.";
    ...
    public function actionUpdatestatus()
    {
        ...
        $order_data = Data::sqlRecords("SELECT `order_number` FROM `newegg_can_order_detail` WHERE `merchant_id`='" . MERCHANT_ID . "'", 'all');
...
//            $url = '/ordermgmt/orderstatus/orders/' . 123456789;
            ...
            if (!empty($res) && !empty($res['OrderStatusName'])) {
                $model = Data::sqlRecords('UPDATE `newegg_can_order_detail` SET order_status_description ="' . $res['OrderStatusName'] . '" WHERE order_number="' . $res['OrderNumber'] . '" AND merchant_id="' . MERCHANT_ID . '" ', null, 'update');
                $count++;
