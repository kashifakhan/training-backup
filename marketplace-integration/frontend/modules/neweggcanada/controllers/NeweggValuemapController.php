            ...
            if(isset($data['newegg']) && !empty($data['newegg'])){
             foreach ($data['newegg'] as $items => $values) {
                ...
                foreach ($values as $key1 => $val1) {
                    ...
                    $select = "SELECT `shopify_product_type` FROM `newegg_can_value_mapping` WHERE `merchant_id`='".$merchant_id."' AND `shopify_product_type`='".addslashes($productType)."'";
                    $prev_data = Data::sqlRecords($select,"one","select");
                    if(empty($prev_data)){
                     $query = "INSERT INTO `newegg_can_value_mapping`(`merchant_id`, `shopify_product_type`,`category_id`,`mapped_value_data`) VALUES ('" . $merchant_id . "','" . addslashes($productType) . "','" . $categoryId . "','" .addslashes($attributeMapData). "') ";
                        Data::sqlRecords($query, null, 'insert');
                    }
                    else{
                        $sql='UPDATE `newegg_can_value_mapping` SET  category_id="'.trim($categoryId).'",mapped_value_data="'.addslashes($attributeMapData).'" where merchant_id="'.$merchant_id.'" and shopify_product_type="'.addslashes($productType).'"';
