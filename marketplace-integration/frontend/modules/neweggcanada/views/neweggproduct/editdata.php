...
$query2 = "SELECT * FROM `newegg_can_product` WHERE `product_id` = '" . $model->product_id . "' AND `merchant_id` =" . MERCHANT_ID;
$neweggproduct = Data::sqlRecords($query2, 'one');
$query = "SELECT `shop_url` from `newegg_can_shop_detail` where `merchant_id`=" . MERCHANT_ID;
                ...
                <div class="jet-pages-heading">
                    ...
                    <div class="product-upload-menu">
                        ...
                        <a class="btn btn-primary"
                           href="<?= Yii::$app->request->baseUrl ?>/neweggcanada/neweggproduct/index">BACK</a>
                ...
                <div class="">
                    <div class="jet-product-form">
                        ...
                        <?php $form = ActiveForm::begin([
                            'id' => 'jet_edit_form',
                            'action' => frontend\modules\neweggcanada\components\Data::getUrl('neweggproduct/productsave/?id=' . $model->product_id),
                            'method' => 'post',
                        ]); ?>
                       ...
                       */ ?>
                            <div class="field-jetproduct table-responsive grid-view">
                                <?php $brand = "";
                            ...
                            <!-- Code By Himanshu -->
                            <div id="category_tab" class="table-responsive grid-view">
...
function mapTwoLevel(check = '',check1 = ''){
    ...
    if(check1.length==0){
        mainreturn = mapOneLevel($('.dropdown_0'));
        return mainreturn
    }
...
function mapThreeLevel(){
    ...
    if($('.dropdown_0').length==0){
        mainreturn = mapOneLevel($('.dropdown_1'),$('.dropdown_2'));
        return mainreturn
    }
    if($('.dropdown_1').length==0){
        mainreturn = mapOneLevel($('.dropdown_0'),$('.dropdown_2'));
        return mainreturn
    }
    if($('.dropdown_2').length==0){
        mainreturn = mapOneLevel($('.dropdown_0'),$('.dropdown_1'));
        return mainreturn
    }
    $.each($('.dropdown_2'), function(key,val1) {
        if($(this).val()){
            if($('#variants_1_'+key+'').find('select').val()){
                if($('#variants_1_'+key+'').find('select').val()){
                    var index = $(this).val()+'_'+$('#variants_1_'+key+'').find('select').val()+'_'+$('#variants_0_'+key+'').find('select').val();
                    if(validator[$(this).val()]){
                        mainreturn['error'][key]=false; 
                    }
                    else{
