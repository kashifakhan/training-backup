...
?>
    <div class="jet-product-index content-section">
        <div class="form new-section">
            ...
            <?php 
            $categoryData=Data::sqlRecords("SELECT `shopify_product_type`,`newegg_category_id` FROM `newegg_can_attribute_map` WHERE `merchant_id`='".MERCHANT_ID."' AND `attribute_map` IS NOT NULL","all","select");
            if(count($categoryData)>0){
            $savedData=Data::sqlRecords("SELECT * FROM `newegg_can_value_mapping` WHERE `merchant_id`='".MERCHANT_ID."'","all","select");
            if(empty($savedData)){
            ?>
                <div class="alert alert-warning">
                  <strong>Map atleast one product value for uploading variants product Just click <a
                        href="<?= yii\helpers\Url::toRoute('newegg-valuemap/index'); ?>"> here </a> to map all shopify variants product value(s) with newegg attribute value. </strong> 
                </div>
            <?php } }?>
      ...
      <i class="fa fa-list" aria-hidden="true"></i>
        <!-- <i class="fa fa-bell fa-1x"></i> -->
            ...
            <div class="submit-upload-wrap">
                ...
                <?php $arrAction = array('batch-upload' => 'Upload', 'image-update' => 'Image Update'); ?>
            ...
            <?= GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'filter' => ["ACTIVATED" => "ACTIVATED", "Not Uploaded" => "Not Uploaded", "SUBMITTED" => "SUBMITTED", "DEACTIVATED" => "DEACTIVATED", "UPLOAD WITH ERROR" => "UPLOAD WITH ERROR", "other" => "Other Products"],
                        'format' => 'html',
                        'value' => function ($data) {
        ...
        $(document).ready(function () {
            ...
            setTimeout(function () {
                ...
                productQuicktour.start().oncomplete(function () {
                    window.location.href = '<?= \frontend\modules\neweggcanada\components\Data::getUrl("neweggorderdetail/index?tour") ?>';
                });
            }, 1000);
