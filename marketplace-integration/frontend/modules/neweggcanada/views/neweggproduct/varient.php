...
<table class="table table-striped table-bordered">
    ...
    <tr>
        ...
        $query1 = "SELECT `attribute_map` FROM `newegg_can_attribute_map` WHERE merchant_id =".MERCHANT_ID." AND shopify_product_type='".$model->shopify_product_type."' AND  newegg_category_id = '".$model->newegg_category."'";
        $array = Data::sqlRecords($query1,"one","select");
        $query = "SELECT * FROM `newegg_can_product_variants` AS neweggvar INNER JOIN `jet_product_variants` AS jetvar ON neweggvar.option_id = jetvar.option_id WHERE jetvar.product_id='$product_id' order by jetvar.option_sku='" . $sku . "' desc, jetvar.option_id asc";
...
<?php       if($data['variant_option1'])
                        {
            ?>
            <td class="variants_0" id='variants_0_<?=$i?>'><input type="text"  value="<?= $data['variant_option1'] ?>" name="jet_attributes[0][<?=$data['option_id']?>][value]" readonly=""></td>
            <?php
                        }
                    if($data['variant_option2'])
                        {
            ?>
            <td class="variants_1" id='variants_1_<?=$i?>'><input type="text"  value="<?= $data['variant_option2'] ?>" name="jet_attributes[1][<?=$data['option_id']?>][value]" readonly=""></td>
            <?php
                        }
                        if($data['variant_option3'])
                        {
            ?>
            <td class="variants_2" id='variants_2_<?=$i?>'><input type="text" value="<?= $data['variant_option3'] ?>" name="jet_attributes[2][<?=$data['option_id']?>][value]" readonly=""></td>
                    ...
                    .done(function (data) {
                        if(data){
                            ...
                            function setDropdown(queue,innerKey){
                                ...
                                var dropdown  = $('<select />', {class:'dropdown_0',name: 'newegg[<?= $model->shopify_product_type ?>]['+category_id+']['+sel_0+']['+input+']' });
                    ...
                    .done(function (data) {
                        if(data){
                            ...
                            function setDropdown(queue,innerKey){
                                ...
                                var dropdown  = $('<select />', {class:'dropdown_1',name: 'newegg[<?= $model->shopify_product_type ?>]['+category_id+']['+sel_1+']['+input+']' });
                                var dropDownData = JSON.parse(queue);
                                 var predefined = '<?= $model->mapped_value_data ?>';
                                var jsonCheck = true;
                                try{
                        ...
                        if(data){
                           $( ".dropdown_2" ).remove();
                            ...
                            function setDropdown(queue,innerKey){
                                ...
                                var dropdown  = $('<select />', {class:'dropdown_2',name: 'newegg[<?= $model->shopify_product_type ?>]['+category_id+']['+sel_2+']['+input+']' });
                                var predefined = '<?= $model->mapped_value_data ?>';
                                var jsonCheck = true;
                                try{
                                    var objpredefined = JSON.parse(predefined);
