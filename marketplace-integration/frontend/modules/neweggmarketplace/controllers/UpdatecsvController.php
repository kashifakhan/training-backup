...
class UpdatecsvController extends NeweggMainController
{
    ...
    public function actionExport()
    {
        ...
        if (!file_exists(\Yii::getAlias('@webroot') . '/var/newegg/csv_export/' . $merchant_id . '/' . $action)) {
            mkdir(\Yii::getAlias('@webroot') . '/var/newegg/csv_export/' . $merchant_id . '/' . $action, 0775, true);
        }
        $base_path = \Yii::getAlias('@webroot') . '/var/newegg/csv_export/' . $merchant_id . '/' . $action . '/' . $action . '.csv';
...
//        var_dump(in_array('all',$status));die;
        if (in_array('all', $status)) {
            $model = Data::sqlRecords("SELECT * FROM (SELECT * FROM `jet_product` WHERE `merchant_id`='" . $merchant_id . "') as `jp` INNER JOIN (SELECT * FROM `newegg_product` WHERE `merchant_id`='" . $merchant_id . "') as `wp` ON `jp`.`id`=`wp`.`product_id` WHERE `wp`.`merchant_id`= '" . $merchant_id . "'", 'all');
        } else {
            $status1 = implode("','", $status);

            $model = Data::sqlRecords("SELECT * FROM (SELECT * FROM `jet_product` WHERE `merchant_id`='" . $merchant_id . "') as `jp` INNER JOIN (SELECT * FROM `newegg_product` WHERE `merchant_id`='" . $merchant_id . "' AND `upload_status` IN ('" . $status1 . "')) as `wp` ON `jp`.`id`=`wp`.`product_id` WHERE `wp`.`merchant_id`= '" . $merchant_id . "'", 'all');
        }
        ...
        foreach ($model as $value) {
            ...
            if ($value['type'] == "simple") {
                ...
                if (in_array('all', $status)) {
                    $optionResult = Data::sqlRecords("SELECT * FROM (SELECT * FROM `jet_product_variants` WHERE `merchant_id`='" . $merchant_id . "') as `jpv` INNER JOIN (SELECT * FROM `newegg_product_variants` WHERE `merchant_id`='" . $merchant_id . "') as `wpv` ON `jpv`.`option_id`=`wpv`.`option_id` WHERE `wpv`.`merchant_id`= '" . $merchant_id . "' AND `wpv`.`product_id`='" . $value['product_id'] . "'", 'all');
            ...
            foreach ($res['ResponseBody']['ResponseList'] as $feed) {
                if ($feed['RequestStatus'] == 'SUBMITTED') {
                    ...
                    foreach ($prod as $value) {
                        ...
                        if (!empty($ids)) {
                            $query1 = "UPDATE `newegg_product` SET  
