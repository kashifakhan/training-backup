...
class ProductcsvController extends NeweggMainController
{
    ...
    public function actionExport()
    {
        ...
        if (in_array('all', $action)) {
            ...
            $model = Data::sqlRecords("SELECT * FROM (SELECT * FROM `jet_product` WHERE `merchant_id`='" . $merchant_id . "') as `jp` INNER JOIN (SELECT * FROM `newegg_product` WHERE `merchant_id`='" . $merchant_id . "') as `np` ON `jp`.`id`=`np`.`product_id` WHERE `np`.`merchant_id`= '" . $merchant_id . "'", 'all');

        } else {
            $status = implode("','", $action);
            $query = "SELECT * FROM (SELECT * FROM `jet_product` WHERE `merchant_id`='" . $merchant_id . "') as `jp` INNER JOIN (SELECT * FROM `newegg_product` WHERE `merchant_id`='" . $merchant_id . "' AND `upload_status` IN ('" . $status . "')) as `wp` ON `jp`.`id`=`wp`.`product_id` WHERE `wp`.`merchant_id`= '" . $merchant_id . "'";
        ...
        if (isset($_FILES['csvfile']['name'])) {
            ...
            if (($handle = fopen($target, "r"))) {
                ...
                while (($data = fgetcsv($handle, 90000, ",")) !== FALSE) {
                    ...
                    $productData['title'] = htmlspecialchars(addslashes($pro_title));
                    $productData['weight'] = $pro_weight;
                    $productData['manufacturer'] = $pro_manufacturer;
                    $productData['bullet_description'] = htmlspecialchars(addslashes($pro_bullet_description));
                    $productData['long_description'] = htmlspecialchars(ProductApi::changeDescription(addslashes($pro_long_description)));
    ...
    public function actionBatchupdate()
    {
        ...
        foreach ($products['products'] as $product) {
            ...
            if ($product['type'] == 'No Variants') {
                ...
                $when_title .= ' WHEN ' . $product['id'] . ' THEN ' . '"' . htmlspecialchars(addslashes($product['title'])) . '"';
                ...
                $when_bullet_description .= ' WHEN ' . $product['id'] . ' THEN ' . '"' . htmlspecialchars(addslashes($product['bullet_description'])) . '"';
                $when_long_description .= ' WHEN ' . $product['id'] . ' THEN ' . '"' . htmlspecialchars(ProductApi::changeDescription(addslashes($product['long_description']))) . '"';
            }
