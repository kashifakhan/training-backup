...
use frontend\modules\neweggmarketplace\components\Createwebhook;
use Yii;
use yii\helpers\Url;
...
class SiteController extends Controller
{
    ...
    {
        $this->layout = 'main';
        if (isset(Yii::$app->controller->module->module->requestedRoute) && Yii::$app->controller->module->module->requestedRoute == 'neweggmarketplace/site/guide') {
            ...
            Yii::$app->view->registerMetaTag([
                'name' => 'og:url',
                'content' => 'https://shopify.cedcommerce.com/integration/neweggmarketplace/'
            ], "og:url");
            ...
            Yii::$app->view->registerLinkTag([
                'rel' => 'canonical',
                'href' => 'https://shopify.cedcommerce.com/integration/neweggmarketplace/'
            ], true);
        ...
        if (Yii::$app->user->isGuest) {
            ...
            if (!defined('MERCHANT_ID') || Yii::$app->user->identity->id != MERCHANT_ID) {
                ...
                /*if ($shopDetails['install_status'] == 0 || $shopDetails['purchase_status'] == 'uninstall') {
                    return $this->redirect(Yii::$app->getUrlManager()->getBaseUrl().'/neweggmarketplace/site/login');
                }*/
                $neweggConfig = [];
            ...
            //Code By Ankit End
            /*if(Data::getStore(Yii::$app->user->identity->username)){
                    return $this->render('index');
            }else{
                return $this->redirect(['/neweggcanada/site/index']);
            }*/
            return $this->render('index');
        }
    }
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $shopifyClient = new ShopifyClientHelper($shop, "", NEWEGG_APP_KEY, NEWEGG_APP_SECRET);
            ...
            $shopifyClient = new ShopifyClientHelper($_GET['shop'], "", NEWEGG_APP_KEY, NEWEGG_APP_SECRET);
...
//                Data::createWebhooks($sc, $_GET['shop'], $_SESSION['token']); // Creating Webhook
                ...
                if (!$merchant) {
                    ...
                    $merchantModel->app_name = Data::APP_NAME_NEWEGG_US;
                    $merchantModel->save(false);

                    $merchant_id = $merchantModel->merchant_id;
                } else {
                    $merchant->app_name = $merchant->app_name. ',' .Data::APP_NAME_NEWEGG_US;
                    $merchant->save(false);
        ...
        if (isset($_GET['charge_id']) && isset($_GET['plan']) && $_GET['plan'] == 2) {
            ...
            if (isset($response['id']) && $response['status'] == "accepted") {
                ...
                if (is_array($response) && count($response) > 0) {
                    $recurring = "";
                    $recurring = Data::sqlRecords('select `id` from `newegg_payment` where id="' . $_GET['charge_id'] . '"', 'all');
                    if (!$recurring) {
                        ...
                        $query = "insert into `newegg_payment`
                        ...
                        $query = "UPDATE `newegg_shop_detail` SET `purchase_date`='" . $updated_at . "',`expire_date`='" . $expire_date . "' ,`purchase_status`='Purchased' where merchant_id='" . $merchant_id . "'";
