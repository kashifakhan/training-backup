...
use frontend\modules\neweggmarketplace\models\NeweggConfigurationSearch;
...
class NeweggconfigurationController extends NeweggMainController
{
    ...
    public function actionIndex()
    {
        ...
        $data = Data::sqlRecords("SELECT * FROM `newegg_configuration` WHERE `merchant_id`='" . MERCHANT_ID . "'", 'one');

        if ($postData = Yii::$app->request->post()) {

            $query = "SELECT * FROM `newegg_email_template`";
            ...
            if (!Neweggappdetail::validateApiCredentials($seller_id, $secret_key, $authorization)) {
                ...
                $isConfigurationExist = Data::sqlRecords("SELECT `seller_id` FROM  newegg_configuration WHERE `merchant_id`='" . MERCHANT_ID . "' ", "one", "select");
                if (!empty($isConfigurationExist)) {
                    Data::sqlRecords("UPDATE `newegg_configuration` SET `seller_id`='" . $seller_id . "',`secret_key`='" . $secret_key . "',`authorization`='" . $authorization . "' where `merchant_id`='" . MERCHANT_ID . "'", null, "update");
                } else {
                    //save api credentials
                    Data::sqlRecords("INSERT INTO `newegg_configuration` (`merchant_id`, `seller_id`,`secret_key`,`authorization`) values(" . MERCHANT_ID . ",'" . $secret_key . "','" . $authorization . "','" . $authorization . "') ", null, "insert");
                }
            }

            if (!empty($emailConfiguration)) {
                foreach ($emailConfiguration as $key => $value) {
                    Data::sqlRecords("UPDATE `newegg_config` SET `value`='" . $value . "' where `merchant_id`='" . MERCHANT_ID . "' AND `data`='" . $key . "'", null, "update");
                }
            }
