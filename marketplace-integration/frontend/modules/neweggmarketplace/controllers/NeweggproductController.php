...
use frontend\modules\neweggmarketplace\models\NeweggProductSearch;
use frontend\modules\neweggmarketplace\components\product\Productimports;
use frontend\modules\neweggmarketplace\components\product\Productimport;
use frontend\modules\neweggmarketplace\components\product\ProductInventory;
use frontend\modules\neweggmarketplace\components\product\ProductApi;
...
//        print_r($values);die('ssss');
    ...
    public function actionProductsave($id)
    {
        ...
        if (Yii::$app->request->post()) {
                        ...
                        if ($model2) {
                            ...
                            if ($model3 !== "") {
                                ...
                                $sql = 'UPDATE `newegg_product_variants` SET
                                    ...
                                    newegg_option_attributes="' . addslashes($options_save) . '"
                                    where option_id="' . $option_id . '"';
                                Data::sqlRecords($sql, null, "update");
                            }
                        }
            ...
            if (Yii::$app->request->post('newegg')) {
                ...
                $sql = 'UPDATE `newegg_product` SET  mapped_value_data="' . addslashes($attributeMapData) . '" where merchant_id="' . $merchant_id . '" and product_id="' . $product_id . '"';
                Data::sqlRecords($sql, null, "update");
            }
    ...
    public function actionAjaxBulkUpload()
    {
        ...
        } elseif ($action == 'retire-product') {

            $pages = (int)(ceil($Productcount / $size_of_request));
            $selectedProducts = array_chunk($selection, $size_of_request);
            $session->set('retireproduct', $selectedProducts);
            ...
            try {
                ...
                if (isset($productResponse['erroredSkus'])) {
                    foreach ($productResponse['erroredSkus'] as $productSku => $error) {
                        ...
                        $query = "UPDATE `newegg_product` ngp JOIN `jet_product` jp ON ngp.product_id=jp.id SET ngp.`error`='" . addslashes($error) . "' where jp.sku='" . $productSku . "'";
                        Data::sqlRecords($query, null, "update");
                    }
        ...
        foreach ($requiredAttrValues as $key1 => $value1) {
            foreach ($value1 as $key => $value) {
                if ($value['PropertyName'] == $mainData['name']) {
                    $val = \frontend\modules\neweggmarketplace\components\categories\Categoryhelper::subcategoryAttributeValue($key1, $value['SubcategoryID'], $value['PropertyName']);
    ...
    public function actionUpdateprice()
    {
        ...
        $query = 'select jet.id,jet.sku,jet.type,jet.price,jet.merchant_id from `newegg_product` ngg INNER JOIN `jet_product` jet ON jet.id=ngg.product_id where ngg.upload_status!="Not Uploaded" and ngg.merchant_id="' . MERCHANT_ID . '"';
    ...
    public function actionSaveDescription()
    {
        ...
        if ($product_id && $description && is_numeric($product_id)) {
            ...
            if ($length > $maxLength) {
                ...
                $query = "UPDATE `newegg_product` SET `long_description`='" . addslashes($description) . "' WHERE `product_id`='" . $product_id . "'";
                Data::sqlRecords($query, null, 'update');
    ...
    public function actionUpdateinventory()
    {
        ...
        $query = 'select jet.id,jet.sku,jet.type,jet.qty,jet.merchant_id from `newegg_product` ngg INNER JOIN `jet_product` jet ON jet.id=ngg.product_id where ngg.upload_status!="Not Uploaded" and ngg.merchant_id="' . MERCHANT_ID . '"';
        $product = Data::sqlRecords($query, "all", "select");
...
//        $product_ids = Yii::$app->request->post('product_id', false);
        ...
        $index = Yii::$app->request->post('index');
        $product_ids = isset($session['retireproduct'][$index]) ? $session['retireproduct'][$index] : [];
        $count = count($product_ids);

        $response = ProductApi::deleteProduct($product_ids, MERCHANT_ID);
        return $response;
    }

    /*
     * Delete Product from Edit-Section
     */
    ...
    public function actionRetireProduct()
    {
        ...
        if (count($product_ids) > 0) {
            ...
            $response = ['error' => true, 'message' => 'No Products Selected'];
    ...
    public function actionSyncproductstore()
    {
        ...
        $countProducts = 0;
        $pages = 0;
        $sc = new ShopifyClientHelper($shopname, $token, NEWEGG_APP_KEY, NEWEGG_APP_SECRET);
    ...
    {
        $session = Yii::$app->session;
        $query = 'select `sku`,`type`,`product_id` from (SELECT * FROM `newegg_product` WHERE `merchant_id`= "' . MERCHANT_ID . '") as ngg INNER JOIN (SELECT * FROM `jet_product` WHERE `merchant_id`= "' . MERCHANT_ID . '") as jet ON jet.id=ngg.product_id where ngg.merchant_id="' . MERCHANT_ID . '"';
        $product = Data::sqlRecords($query, "all", "select");
        $Productcount = count($product);
        //die("jjj");
        if (is_array($product) && $Productcount) {
    ...
    public function actionCountvariants()
    {
        ...
        /*$products = $sc->call('GET','/admin/products.json?product_type=Office Accessories');
        var_dump($product);die;*/

        if (!isset($products['error'])) {
            $file_dir = \Yii::getAlias('@webroot') . '/var/producttype/';
            if (!file_exists($file_dir)) {
                mkdir($file_dir, 0775, true);
            }
            $filenameOrig = "";
            $filenameOrig = $file_dir . '/producttype.log';
            $fileOrig = "";
            $fileOrig = fopen($filenameOrig, 'a');

            $type = [];
            foreach ($products as $prod) {
                $type[$prod['product_type']] = 1;
