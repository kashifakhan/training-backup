                                ...
                                foreach ($attribute['neweggattribute'] as $value) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="attrmap-inner-table-inner-td1">
                                            ...
                                            <!--                                                            ][-->
                                            <? //= $attribute['neweggcategoryId']?><!--]-->
                                        <?php if($value['IsGroupBy']==1 && $value['IsRequired']==1){?>
                                        <td class="attrmap-inner-table-inner-td2">
                                            <select class="form-control root"
                                                name="newegg[<?= trim($attribute['product_type'], ' ') ?>][<?= $attribute['neweggcategoryId'] ?>][<?= $value['PropertyName'] ?>][<?= AttributeMap::VALUE_TYPE_MIXED ?>]">
                                        ...
                                        elseif($value['IsGroupBy']==1){ ?>
                                            <td class="attrmap-inner-table-inner-td2">
                                               <select class="form-control root"
                                                name="newegg[<?= trim($attribute['product_type'], ' ') ?>][<?= $attribute['neweggcategoryId'] ?>][<?= $value['PropertyName'] ?>][<?= AttributeMap::VALUE_TYPE_SHOPIFY ?>]">
                                                <?php foreach ($items['PropertyValueList'] as $options) {
                                                    ?>
                                                    <option
                                                        value='<?= $options; ?>' <?php
                                       ...
                                       <?php }
                                          ...
                                          <td class="attrmap-inner-table-inner-td2">
                                            <select class="form-control root"
                                                name="newegg[<?= trim($attribute['product_type'], ' ') ?>][<?= $attribute['neweggcategoryId'] ?>][<?= $value['PropertyName'] ?>][<?= AttributeMap::VALUE_TYPE_NEWEGG ?>]">
                                                <?php foreach ($items['PropertyValueList'] as $options) {
                                                    ?>
                                                    <option
                                                        value='<?= $options; ?>' <?php
                                            ...
                                            if (isset($shopifyoptions) && count($shopifyoptions)) {
                                                ...
                                                if (isset($attribute['shopify_attribute']) && $attribute['shopify_attribute']) {
                                                    ...
                                                    foreach ($attribute['shopify_attribute'] as $k => $v) {
                                                        ?>
                                                        <!--               <label><?= $v ?></label>
                                                                <input type="checkbox" value="<?= $v ?>" name="newegg[<?= $name ?>][<?= $attribute['neweggcategoryId'] ?>][<?= $value['PropertyName'] ?>][map][]"<?php if (isset($attribute['mapped_Datas'][$value['PropertyName']])
                                                            && !empty($attribute['mapped_Datas'][$value['PropertyName']])
                                                        ) {
                                                            if (in_array($v, explode(',', $attribute['mapped_Datas'][$value['PropertyName']]))) {
                                                                echo 'checked="checked"';
    ...
    $(document).ready(function(){
      ...
      /*  var attribute = <?=$attributeValue?>;
        ...
        $.each(attribute, function(ekey,evalue) {
            console.log(ekey);
            console.log(evalue);
            $('#'+count+'').append("<td>"+evalue.product_type+"</td><td>"+evalue.neweggcategory+"</td>");
