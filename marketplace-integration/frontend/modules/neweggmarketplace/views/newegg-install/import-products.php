...
use frontend\modules\neweggmarketplace\controllers\NeweggproductimportController;

$this->title = 'Import Products';
$merchant_id=Yii::$app->user->identity->id;
$shopname = Yii::$app->user->identity->username;
$shop_url = !empty($shopname) ? trim($shopname) : "";
$urlTotal = Data::getUrl('neweggproductimport/gettotaldetails');
$url = Data::getUrl('neweggproductimport/batchimport');
$varUrl = yii::$app->request->baseUrl.'/var/newegg/importproduct/'.$merchant_id.'/';
$viewUrl = Data::getUrl('neweggproductimport/viewerroredproduct');
$file_content = Data::getUrl('neweggproductimport/getfilecontent');
$file_exist = Data::getUrl('neweggproductimport/fileexist');
...
<script>
    ...
    function loopTotalDetails(flag, url, page, selectVal, non_sku_total=0, non_type_total=0,total_variants_prod=0){
        ...
        $.ajax({
            ...
            async: false,
            dataType: 'json',
            data: {select:selectVal, limit:limit, page:page,non_sku_total:non_sku_total,non_type_total:non_type_total,total_variants_prod: total_variants_prod, merchant_id : merchant_id,_csrf : csrfToken}
        })
            .done(function(msg){
                //console.log(msg);
...
//						console.log(total_variants_prod);
    ...
    function startCustomProductImport(element,page=0)
    {
        ...
        if(selectCount)
        {
            ...
            var max = <?= NeweggproductimportController::MAX_CUSTOM_PRODUCT_IMPORT_PER_REQUEST ?>;
    ...
    function sendCustomImportAjax(productIds, page)
    {
        ...
        var url = "<?= Data::getUrl('neweggproductimport/custom-import') ?>";
        $.ajax({
            method: "post",
            url: url,
            dataType: 'json',
