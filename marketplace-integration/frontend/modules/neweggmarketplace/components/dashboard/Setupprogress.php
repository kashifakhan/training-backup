...
class Setupprogress extends Component
{
    ...
    public function getApiStatus($merchant_id)
    {
        if(is_null($this->_apiStatus))
        {
            ...
            if(is_numeric($merchant_id)) {
                $query = "SELECT COUNT(*) as `count` FROM `newegg_configuration` WHERE `merchant_id`=".$merchant_id." LIMIT 0,1";
    ...
    public function getProductImportStatus($merchant_id)
    {
        ...
        {
            if(is_numeric($merchant_id)) {
                $query = "SELECT COUNT(*) as `count` FROM `newegg_product` INNER JOIN `jet_product` ON `newegg_product`.`product_id`=`jet_product`.`id` WHERE `newegg_product`.`merchant_id`=".$merchant_id." LIMIT 0,1";
                $result = Data::sqlRecords($query, 'one');
                if(isset($result['count']) && $result['count']) {
    ...
    public function getCategoryMapStatus($merchant_id)
    {
        ...
        {
            if(is_numeric($merchant_id)) {
                $query = "SELECT COUNT(*) as `count` FROM `newegg_category_map` WHERE `merchant_id`=".$merchant_id." AND  `category_id` != '' AND `category_id` IS NOT NULL LIMIT 0,1";
                $result = Data::sqlRecords($query, 'one');
                if(isset($result['count']) && $result['count']) {
    ...
    public function getAttributeMapStatus($merchant_id)
    {
        ...
        {
            if(is_numeric($merchant_id)) {
                $query = "SELECT COUNT(*) as `count` FROM `newegg_attribute_map` WHERE `merchant_id`=".$merchant_id." LIMIT 0,1";
                $result = Data::sqlRecords($query, 'one');
                if(isset($result['count']) && $result['count']) {
