...
class VariantsProduct extends Component
{
    ...
    */
    ...
    public static function getVariantsSubCategoryProperty($productArray)
    {
        ...
        {
            /*print_r($productArray['newegg_data']);die;*/
            ...
            else
            {
                ...
                $map = [];
                $sql = "SELECT `mapped_value_data` FROM `newegg_value_mapping` WHERE merchant_id='".MERCHANT_ID."' AND shopify_product_type='".$productArray['shopify_product_type']."' AND category_id='".$productArray['category_id']."'";
                $array = Data::sqlRecords($sql,"one","select");
                $query = 'SELECT `attribute_map`,`attribute_map_data` from newegg_attribute_map Where `shopify_product_type`="'.$productArray['shopify_product_type'].'" and `newegg_category_id`="'.$productArray['category_id'].'"';
