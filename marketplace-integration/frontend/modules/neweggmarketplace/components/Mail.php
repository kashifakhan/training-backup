...
class Mail extends MailSources
{
	...
	public function getFile(){
		...
		$file = dirname(Yii::getAlias('@webroot')).'/frontend/modules/neweggmarketplace/views/templates/'.$this->_template;
		$serach[]=$file;
		if(file_exists($file)){
			return $file;
		}

		$file = Yii::getAlias('@webroot').'/frontend/modules/neweggmarketplace/views/templates/'.$this->_template;
		$serach[]=$file;
		if(file_exists($file)){
			return $file;
		}
		...
		try
		{
			if(isset($this->_data['merchant_id']) && !empty($this->_data['merchant_id']))
			{
				$template = str_replace('.html', '', $this->_template);
				 $query = "SELECT `value` FROM  `newegg_config` WHERE `merchant_id`='".$this->_data['merchant_id']."' AND `data`='".$template."'";
	...
	public function insertTemplate($template){
		$query="SELECT `template_path` FROM `newegg_email_template` WHERE template_path='".$template."'";
	    $allData = Data::sqlRecords($query, 'one');
	    if(empty($allData)){
	    	$mainTitle =str_replace('.html', '',$template);
	    	$title =str_replace('email/', '',$mainTitle);
	    	
	    	$model = Data::sqlRecords("INSERT INTO `newegg_email_template` (`template_title`,`template_path`,`custom_title`) VALUES ('".$title."','".$template."','".$title."')", 'all','insert');
	    	if(isset($this->_data['merchant_id'])){
	    		$model = Data::sqlRecords("INSERT INTO `newegg_config` (`data`,`value`,`merchant_id`) VALUES ('".$mainTitle."','1','".$this->_data['merchant_id']."')", 'all','insert');
	    	}
