        ...
        */?>
		<div class="wrap ced-jet-navigation-mbl">
			<div class="fixed-container-body-class">
				...
				<div class="col-sm-6 plateform-switch-body no-padding">
					<?php
					$merchant_id = Yii::$app->user->identity->id;
					$controllerArray = ['site', 'jetconfiguration', 'categorymap', 'jetproduct', 'jet-attributemap', 'jetorderdetail', 'jetorderimporterror'];
					if (in_array(Yii::$app->controller->id, $controllerArray)) {
						$walsql = $walpath = $newpath = $walpathurl = "";
						$isWalmartClient = $isNeweggClient = [];
						$walsql = "SELECT `id` FROM `walmart_shop_details` WHERE `merchant_id`='{$merchant_id}' ";
						$isWalmartClient = Data::sqlRecords($walsql, 'one', 'select');
						...
						if (empty($isWalmartClient)) {
							$walpath = '<a href="https://apps.shopify.com/newegg-marketplace-integration" target="_blank"><button class="btn-path">Install Now</button></a>';
						} else {
							$newpath = preg_replace('/jet/', 'walmart', Yii::$app->controller->id);
							$walpathurl = Yii::getAlias('@webwalmarturl') . "/" . $newpath . "/index?to_switch=walmart";
							$walpath = '<a href="' . $walpathurl . '" target="_blank"><button class="btn-path">Switch Store</button></a>';
						}

						if (empty($isNeweggClient)) {
							$eggpath = '<a href="https://apps.shopify.com/newegg-marketplace-integration" target="_blank"><button class="btn-path">Install Now</button></a>';
						} else {
							$neweggpath = preg_replace('/jet/', 'newegg', Yii::$app->controller->id);
							$eggpathurl = Yii::getAlias('@webneweggurl') . "/" . $neweggpath . "/index?to_switch=newegg";
							$eggpath = '<a href="' . $eggpathurl . '" target="_blank"><button class="btn-path">Switch Store</button></a>';
						}
						?>
