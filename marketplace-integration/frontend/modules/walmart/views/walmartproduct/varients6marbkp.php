...
?>
		<script type="text/javascript">
            ...
            function getCommonAttributeValue(attr)
            {
            	...
            	<?php foreach ($common_attr_values as $common_key => $common_value):
		...
		<table class="table table-striped table-bordered">
			...
			?>
					<td>
						...
						<select class="variant_attributes_selector jet_attributes_selector form-control" id="sel_<?=$key?>" name="jet_attributes[<?=$key?>][jet_attr_id]">
			...
			<?php 				if(is_array($dropdown_option_arr) && count($dropdown_option_arr)>0)
								{
									...
									{
			?>
										<option value="<?=trim($p_ar['value'])?>" <?php if(trim($p_ar['value'])==trim($attr_value)){echo "selected='selected'";}?>>
											<?=$p_ar['name']?>
										</option>
			<?php 					}
								}
			...
			?>
									<div <?php if(trim($p_ar['name'])==$attr_value){echo "style='display:block;'";}else{echo "style='display:none;'";}?> class="unit_dropdown_div_class unit_dropdown_div_<?=$key?>_<?=$p_ar_value?>">
										<label class="unit_label" style="width:30%;float:left;">Unit </label>
										<select <?php if(trim($p_ar['name'])!=$attr_value){echo "disabled='disabled'";}?> style="width:50%;" class="unit_dropdown form-control" id="opt_<?=$key?>_<?=$p_ar_value?>" name="common_attributes[<?=$p_ar['select_name']?>]">
			<?php 							foreach($p_ar['select'] as $ky=>$sel_op)
											{
			?>
											<option value="<?=trim($sel_op)?>" <?php if(trim($p_ar['select_value'])==trim($sel_op) && trim($p_ar['name'])==$attr_value){echo "selected='selected'";}?>>
												<?=trim($sel_op)?>
											</option>
			<?php 							}	
			?>
    		...
    		?>
    				<tr>
    					...
    					<td>
    					   <?= $value['option_price'] ?>&nbsp;&nbsp;
    					   <a href="javascript:void(0);" onclick="priceEdit(this)" class="variant-price" product-id="<?= $value['product_id'] ?>" option-id="<?= $value['option_id'] ?>" sku="<?= $value['option_sku'] ?>" option-price="<?= $value['option_price'] ?>">Add Promotional Price</a>
    					</td>
    					<td>
    					   <?=$value['option_qty']?>
