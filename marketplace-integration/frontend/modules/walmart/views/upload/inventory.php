...
use frontend\modules\walmart\components\Data;
	...
	$url = Data::getUrl('upload/startinventoryupload');
?>
<style type="text/css" >
        ...
        <div class="panel panel-default form new-section">
            <div class="jet-pages-heading">
                    <div class="title-need-help">
                        <h1 class="Jet_Products_style">Inventory Upload</h1>
                    </div>
...
<script type="text/javascript"> 
	...
	function uploaddata(){
		...
		$.ajax({
            ...
            method: "post",
            dataType:'json',
            data:{ index : id,_csrf : csrfToken, csvFilePath : "<?= $csvFilePath ?>"},
        	success: function(json) {
				...
				if(json.success) {
					...
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span class="text">'+json.success_count+' Product(s) Submitted of page '+id+'.</span>';
					span.id = 'id-'+id;
				...
				if(json.error)
				{
							...
							$.each(json.error_msg, function(sku, msg) {
								...
								if(typeof msg === "object") {
									if(msg.hasOwnProperty('xml_validation_error')) {
										var span = document.createElement('li');
										span.innerHTML = '<img class="v-middle" src="<?php echo $error_img ?>"><span class="text">'+sku+' : '+msg.error+'</span>';
										span.style = 'background-color:#FDD';
										update_row.parentNode.insertBefore(span, update_row); 
									} 
									else {
										$.each(msg, function(variantSku, variantError) {
											if(typeof variantError === "object") {
												var span = document.createElement('li');
												span.innerHTML = '<img class="v-middle" src="<?php echo $error_img ?>"><span class="text">Variant : '+variantSku+' : '+variantError.error+'</span>';
												span.style = 'background-color:#FDD';
												update_row.parentNode.insertBefore(span, update_row); 
											}
											else {
												var span = document.createElement('li');
												span.innerHTML = '<img class="v-middle" src="<?php echo $error_img ?>"><span class="text">Variant : '+variantSku+' : '+variantError+'</span>';
												span.style = 'background-color:#FDD';
												update_row.parentNode.insertBefore(span, update_row); 
											}
										});
					...
					if(json.hasOwnProperty('feedError') && json.feedError.hasOwnProperty('error'))
					{
						...
						'Feed Can not be Submitted to Walmart (Error Code : '+json.feedError.error.code+', Error Info : '+json.feedError.error.info+')</span>';
						span.style = 'background-color:#FDD';
						update_row.parentNode.insertBefore(span, update_row);
					}
				}
				...
				else 
				{
					...
					var span = document.createElement('li');
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product(s) Successfully Submitted to Walmart.'+'</span>';
					span.style = 'background-color:#DDF';
				...
				<?php if(!empty($after_upload) && $after_upload) : ?>
					afterUpload();
				<?php endif; ?>
				}
            },
    ...
    {
    	$.ajax({
            url:"<?= Data::getUrl('walmartproductfeed/after-reupload') ?>",
            method: "post",
            dataType:'json',
            data:{ start : '1',_csrf : csrfToken},
        	success: function(json) {
        		console.log(json);
        	}
        });
