...
(function(factory){
    ...
    Tokenize2.prototype.bind = function(){
        ...
        this.element.on('tokenize:load', {}, $.proxy(function(){ this.init() }, this))
            ...
            .on('tokenize:tokens:addAll', {}, $.proxy(function(){ this.tokenAddAll() }, this))
            .on('tokenize:tokens:removeAll', {}, $.proxy(function(){ this.clear() }, this));
    };
     ...
     * @returns {Tokenize2}
     */
    Tokenize2.prototype.tokenAddAll = function(){
         $.each($('li.dropdown-item', this.dropdown), $.proxy(function(e, item){
            if($(item).find('a').attr('data-value') != undefined) {
                this.trigger('tokenize:tokens:add', [$(item).find('a').attr('data-value'), $(item).find('a').attr('data-text'), true]);
            }
        }, this));
    ...
    /*Tokenize2.prototype.dropdownShow = function(){
        ...
        if(!this.isDropdownOpen()){
            ...
            var actionWrapper = $('<div class="custom-action-buttons"></div>');

            var selectAllAction = $('<a class="sol-select-all" href="#">Select all</a>')
                                    .on('mousedown touchstart', {}, $.proxy(function(e){
                                        e.preventDefault();
                                        this.trigger('tokenize:tokens:addAll');
                                    }, this));

            var selectNoneAction = $('<a class="sol-deselect-all" href="#">Select none</a>')
                                    .on('mousedown touchstart', {}, $.proxy(function(e){
                                        e.preventDefault();
                                        this.trigger('tokenize:tokens:removeAll');
                                    }, this));

            actionWrapper.append(selectAllAction);
            actionWrapper.append(selectNoneAction);

            this.dropdown = $('<div class="tokenize-dropdown dropdown"></div>').attr('data-related', this.id);

            this.dropdown.append(actionWrapper);

            this.dropdown.append('<ul class="dropdown-menu" />');
    ...
    Tokenize2.prototype.dropdownFill = function(items){
        ...
        if(items && items.length > 0){
            ...
            if(this.options.displayNoResultsMessage){
                ...
                var searchString = this.input.val().trim();
                if(searchString == '') {
                    var message = 'No records found.';
                } else {
                    var message = this.options.noResultsMessageText.replace('%s', this.input.val());
                }
                this.dropdown.find('.dropdown-menu').append(
                    $('<li class="dropdown-item locked" />').html(message)
                );
            } else {
                this.trigger('tokenize:dropdown:hide');
            }
     ...
     */
    Tokenize2.prototype.Addactions = function(){

        var actionWrapper = $('<div class="custom-action-buttons"></div>');

        var selectAllAction = $('<a class="select-all" href="javascript:void(0);">All</a>')
                                .on('mousedown touchstart', {}, $.proxy(function(e){
                                    e.preventDefault();
                                    this.trigger('tokenize:tokens:addAll');
                                }, this));

        var selectNoneAction = $('<a class="select-none" href="javascript:void(0);">None</a>')
                                .on('mousedown touchstart', {}, $.proxy(function(e){
                                    e.preventDefault();
                                    this.trigger('tokenize:tokens:removeAll');
                                }, this));

        actionWrapper.append('<span class="action-label">Select:</span>');
        actionWrapper.append(selectAllAction);
        actionWrapper.append(selectNoneAction);

        this.dropdown.find('.dropdown-menu').append(
            $('<li class="dropdown-item-action" />').html(actionWrapper)
        );
    };
