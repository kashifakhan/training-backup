...
class Mail extends MailSources
{
	...
	public function insertTemplate($template){
	    ...
	    if(empty($allData)){
	    	...
	    	if(isset($this->_data['merchant_id'])){
	    		$model = Data::sqlRecords("INSERT INTO `walmart_config` (`data`,`value`,`merchant_id`) VALUES ('".$mainTitle."','1','".$this->_data['merchant_id']."')", 'all','insert');
	    	}
   ...
   private function _addhtmlImage($matches){
   		$path = Yii::getAlias('@weburl')."/admin/reports/client-email-read/index?id=".$this->_tracking;
