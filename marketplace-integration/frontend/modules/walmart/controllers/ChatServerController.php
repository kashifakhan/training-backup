...
{
  const DEFAULT_CHAT_TIMEZONE = 'America/New_York';
  const DEFAULT_CHAT_DATE_FORMAT = 'F j, Y';//July 1, 1999
  const DEFAULT_CHAT_TIME_FORMAT = 'H:i:s';//20:05:30
  ...
  public function actionIndex($data=false) 
  {
	...
	{
		$msgTime = self::getCurrentDateTime();
		...
		if($resourceMappingData['sender_type']=='customer'){
			...
			$msgData = array_merge($msgTime, $msgData);
		...
		elseif($resourceMappingData['sender_type']=='group'){
			...
			if(isset($this->liveMembers['customer']) && isset($this->liveMembers['customer'][$messageData['id']]) ){
				...
				$msgData = array('message_type'=>'message', 'user_id'=>$resourceMappingData['group_id'], 'sender_name'=>$resourceMappingData['sender_name'], 'message'=>$messageData['message'], 'receiver_ref'=>$messageData['id']);
				$msgData = array_merge($msgTime, $msgData);
			...
			else
			{
				...
				echo 'Sending to group '.PHP_EOL;
				$this->sendMessageToGroupMembers($resourceMappingData['group_id'],$msgData);
				$this->saveMessage($msgData,['sender_ref'=>$messageData['id']]);
			}
		}
	...
	{
		date_default_timezone_set(self::DEFAULT_CHAT_TIMEZONE);
		$date = date(self::DEFAULT_CHAT_DATE_FORMAT);
		$time = date(self::DEFAULT_CHAT_TIME_FORMAT);
