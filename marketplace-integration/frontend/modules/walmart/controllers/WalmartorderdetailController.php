...
class WalmartorderdetailController extends WalmartmainController
{
    ...
    public function actionIndex()
    {
        ...
        $resultdata = array();
        $queryObj = "";
        $query = "SELECT `shopify_order_id`,`purchase_order_id` FROM `walmart_order_details` WHERE merchant_id='" . $merchant_id . "' AND (status='acknowledged' OR status='Partially Acknowledged') AND shopify_order_id=0";
        $resultdata = Data::sqlRecords($query, 'all');
        /*if ($resultdata) {
            foreach ($resultdata as $order) {
                ...
                $this->walmartHelper = new Walmartapi(API_USER, API_PASSWORD, CONSUMER_CHANNEL_TYPE_ID);
                $orderdata = $this->walmartHelper->getOrder($order['purchase_order_id']);
//                $orderdata = '{"order":{"purchaseOrderId":"4577884097648","customerOrderId":"3061787682911","customerEmailId":"72AE4BDD880A4A7FA276D70FA82A3318@relay.walmart.com","orderDate":1486225007000,"shippingInfo":{"phone":"2288260787","estimatedDeliveryDate":1487142000000,"estimatedShipDate":1486537200000,"methodCode":"Standard","postalAddress":{"name":"Richard Warner","address1":"15600 Omas Rd","address2":null,"city":"Vancleave","state":"MS","postalCode":"39565","country":"USA","addressType":"RESIDENTIAL"}},"orderLines":{"orderLine":[{"lineNumber":"1","item":{"productName":"Silhouette Spatula","sku":"TOOL-03-3T"},"charges":{"charge":[{"chargeType":"PRODUCT","chargeName":"ItemPrice","chargeAmount":{"currency":"USD","amount":0.00},"tax":null},{"chargeType":"SHIPPING","chargeName":"Shipping","chargeAmount":{"currency":"USD","amount":0.00},"tax":null}]},"orderLineQuantity":{"unitOfMeasurement":"EACH","amount":"1"},"statusDate":1486413893000,"orderLineStatuses":{"orderLineStatus":[{"status":"Cancelled","statusQuantity":{"unitOfMeasurement":"EACH","amount":"1"},"cancellationReason":null,"trackingInfo":null}]},"refund":null}]}}}';
                $order_data = json_decode($orderdata, true);
                if ($order_data['order']) {
                    foreach ($order_data['order']['orderLines']['orderLine'] as $orderline) {

                        $status[] = $orderline['orderLineStatuses']['orderLineStatus'][0]['status'];
                        if($orderline['orderLineStatuses']['orderLineStatus'][0]['status'] == 'Acknowledged')
                        {
                            $ack_count++;
                        }
                    }
                }
                if(in_array('Acknowledged',$status) && count($status) == $ack_count )
                {
                    $query = "UPDATE `walmart_order_details` SET status='acknowledged' WHERE purchase_order_id='" . $order_data['order']['purchaseOrderId'] . "'";
                }elseif (in_array('Acknowledged',$status) && count($status) > $ack_count ){
                    $query = "UPDATE `walmart_order_details` SET status='Partially Acknowledged' WHERE purchase_order_id='" . $order_data['order']['purchaseOrderId'] . "'";
                }else{
                    $query = "UPDATE `walmart_order_details` SET status='".$status[0]."' WHERE purchase_order_id='" . $order_data['order']['purchaseOrderId'] . "'";
                }
            ...
            try {
                ...
                if (count($resultdata) > 0) {
                    foreach ($resultdata as $val) {
                        ...
                        $ikey = 0;
                        $result = array();
                        /*$val['order_data']='{"purchaseOrderId":"3579693298119","customerOrderId":"4871791302622",
                                            "customerEmailId":"B79089D9B35247CF9ACF3D9B9E951F4D@relay.walmart.com",
                                            "orderDate":1501858477000,
                                            "shippingInfo":{"phone":"8315883391","estimatedDeliveryDate":1502949600000,
                                            "estimatedShipDate":1502172000000,"methodCode":"Value",
                                            "postalAddress":{"name":"Gary  Bragado","address1":"117 Roache Rd","address2":null,
                                            "city":"Freedom","state":"CA","postalCode":"95019","country":"USA",
                                            "addressType":"RESIDENTIAL"}},
                                            "orderLines":{"orderLine":
                                            [{"lineNumber":"1","item":{"productName":"test545","sku":"test545"},
                                            "charges":{"charge":[{"chargeType":"PRODUCT","chargeName":"ItemPrice",
                                            "chargeAmount":{"currency":"USD","amount":129.99},"tax":
                                            {"taxName":"Tax1","taxAmount":{"currency":"USD","amount":9.42}}}]},
                                            "orderLineQuantity":{"unitOfMeasurement":"EACH","amount":"1"},
                                            "statusDate":1501858535000,"orderLineStatuses":{"orderLineStatus":
                                            [{"status":"Created","statusQuantity":{"unitOfMeasurement":"EACH","amount":"1"},
                                            "cancellationReason":null,"trackingInfo":null}]},"refund":null}]}}';*/
                        ...
                        if (count($order) > 0) {
                            ...
                            foreach ($this->getItems($order['orderLines']['orderLine']) as $key => $value) {
                                ...
                                $itemArray[$ikey]['price'] = $value['price'];

                                if (isset($value['charges']['charge'])) {
                                    foreach ($value['charges']['charge'] as $charge_key => $charge) {
                                        if (isset($charge['tax']) && !empty($charge['tax'])) {
                                            $tax['price'] = $charge['tax']['taxAmount']['amount'];
                                            $tax['rate'] = 0;
                                            $tax['name'] = $charge['tax']['taxName'];
                                            $itemArray[$ikey]['tax_lines'][$charge_key] = $tax;
                                            /*$totalTax += $tax['price'];*/
                                        }
                                    }
                        ...
                        if (count($itemArray) > 0) {
                            ...
                            if ($OrderTotal > 0 && $totalTax > 0) {
                                $Orderarray['order']['total_price'] = $OrderTotal + $totalTax;
                                $Orderarray['order']['total_tax'] = $totalTax;
                                $Orderarray['order']['total_line_items_price'] = $OrderTotal;
                            }
                            if ($shipping > 0 || true) {
                                if ($merchant_id != '335') {
