...
class WalmartproductController extends WalmartmainController
{
    ...
    public function beforeAction($action)
    {
        ...
        if (Yii::$app->requestedAction->id == 'index') 
        {
            ...
            if(isset($query['walmart/walmartproduct/index'])){
                unset($query['walmart/walmartproduct/index']);
            }
            $queryString = http_build_query($query);
            return \Yii::$app->getResponse()->redirect(Data::getUrl('product/index').'?'.$queryString);
        }

        if (parent::beforeAction($action)) {
            $this->walmartHelper = new Walmartapi(API_USER, API_PASSWORD, CONSUMER_CHANNEL_TYPE_ID);
            $this->inventoryMgmtHelper = new InventoryUpdate(API_USER, API_PASSWORD, CONSUMER_CHANNEL_TYPE_ID);
            return true;
        }
    }
    ...
    public function actionEditdata()
    {
        ...
        /* All required attributes ends */

        /* All variants attributes start */
        $variant_attributes = [];
        ...
        foreach ($categoryVariantAttributes as $key => $values) {
            if (array_key_exists($values, $categoryAttributes)) {
                ...
                foreach ($attribute_codes as $items) {
        ...
        foreach ($requiredAttributeCode as $val) {
            if (!in_array($val, $variant_attributes)) {
                $common_required_attributes[] = $val;
            }
        }

        /* Common Required Attributes ends*/

        /* Unit Attributes starts */
        $unitAttributes = [];
        foreach ($categoryAttributes as $key => $value1) {
        ...
        $_attributes = $variant_attributes;

        /* Unit Attributes ends */

        foreach ($_attributes as $key => $attr) {
            if (in_array($attr, $unitAttributes)) {

                unset($_attributes[$key]);
            }
        }

        /* Walmart category Data starts */

        $walmartCode = [];
        $walmartCategoryAttributes = Category::getSubCategoryAttributes($parent_category, $child_category);

        foreach ($walmartCategoryAttributes as $key => $value) {
            $walmartAttributeCode = Category::getAttributeCode($value);

            foreach ($walmartAttributeCode as $item) {
                $walmartCode[] = $item;
            }
        ...
        $Category['walmart_attributes'] = $walmartCode;
        $Category['walmart_attribute_values'] = $attribute_options;

        /* Walmart category Data ends */

        $session = Yii::$app->session;
        $productData = [
            'model' => $model,
            'category_path' => $category_path,
            'attributes' => $_attributes,//all variant attributes
            'optional_attr' => []/*$optional_attr*/,
            'requiredAttrValues' => /*$requiredAttrValues*///attribute option values
                $attribute_options,
            /*'optionalAttrValues' => $optionalAttrValues,*/
            'common_required_attributes' => $common_required_attributes,//required attrs which are not allowed in variant
            'required' => $requiredAttributeCode, // all required attrs
            'unit_attributes' => $unitAttributes,// all unit attrs
...
//        print_r($productData);die;
        ...
        $html = $this->render('editdata', array('id' => $id, 'model' => $model, 'category_path' => $category_path, 'attributes' => $_attributes, 'optional_attr' => []/*$optional_attr*/, 'requiredAttrValues' => $attribute_options, /*'optionalAttrValues' => $optionalAttrValues,*/
            'common_required_attributes' => $common_required_attributes, 'required' => $required), true);
        unset($connection);
        //unset($optional_attr);
        unset($attributes);
        return $html;
...
//                'optionalAttrValues' => $optionalAttrValues,
     ...
     public function actionPromotionSave()
    {
        ...
        try {
            ...
            $post = Yii::$app->request->post();
            if($post['option_id']==''){
                $query = "DELETE FROM `walmart_promotional_price` WHERE `merchant_id`='".$post['merchant_id']."' AND `product_id`='".$post['product_id']."'";
                    Data::sqlRecords($query, null, "delete");
            }else{
                $query = "DELETE FROM `walmart_promotional_price` WHERE `merchant_id`='".$post['merchant_id']."' AND `option_id`='".$post['option_id']."'";
                Data::sqlRecords($query, null, "delete");
            }

            if(isset($post['promotion']['orignal_price']))
            {
                foreach ($post['promotion']['orignal_price'] as $key => $price) {
                    ...
                    if ($effective_date < $expiration_date) {
                        ...
                        /*if (isset($post['promotion']['id']) && isset($post['promotion']['id'][$key])) {
                            ...
                            if(isset($post['promotion']['status'])){
                                if(isset($post['promotion']['status'][$key])){
                                    $pending = $post['promotion']['status'][$key];
                                }else{
    ...
    public function actionUpdateajax($id)
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            $long_description = trim(isset($_POST['long_description']) ? $_POST['long_description'] : '');
            $product_short = trim(isset($_POST['JetProduct']['short_description']) ? $_POST['JetProduct']['short_description'] : '');
            $product_self = trim(isset($_POST['JetProduct']['self_description']) ? $_POST['JetProduct']['self_description'] : '');
...
//                        print_r($option_id);
    ...
    public function actionStartbatchupload()
    {
        ...
        if (!$count) {
            ...
            try {
                ...
                //If some products have been submitted to walmart.
    ...
    public function actionInventorypost()
    {
        ...
        if ($count) {
            ...
            $response = $this->inventoryMgmtHelper->updateInventoryOnWalmart($selectedProducts, "product");
...
//                    $query = "UPDATE `walmart_product` `wp` LEFT JOIN `walmart_product_variants` `wpv` on `wp`.`product_id`=`wpv`.`product_id` SET `wp`.`status`='" . WalmartProduct::PRODUCT_STATUS_NOT_UPLOADED . "',`wpv`.`status`='" . WalmartProduct::PRODUCT_STATUS_NOT_UPLOADED . "' WHERE (`wp`.`status`='" . WalmartProduct::PRODUCT_STATUS_PROCESSING . "' OR `wpv`.`status`='" . WalmartProduct::PRODUCT_STATUS_PROCESSING . "') AND `wp`.`merchant_id`='" . $merchant_id . "'";
    ...
    public function actionGetwalmartdata()
    {
        ...
        $source = Yii::$app->request->post('source', false);
        ...
        if (is_array($resultItems) && isset($resultItems['MPItemView'][0]) && count($resultItems) > 0) 
        {
            ...
            $html = $this->render('view', 
                                array('data' => $result_array, 'modal_id' => $id), 
                                true);
        }
        else
        {
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <p>Either product not uploaded on walmart OR Walmart Api keys are invalid.</p>
                                    </div>
    ...
    public function actionBatchUpdateInventory()
    {
        ...
        if (!$count) {
            ...
            $error = [];
            if (!empty($selection)) {
                $response = $this->inventoryMgmtHelper->batchupdateInventoryOnWalmart($selection, "product");
                if (isset($response['feedId'])) {
                    if (isset($response['erroredSkus'])) {
                        $returnArr = ['success' => true, 'count' => $count - $response['error_count'], 'erroredSkus' => json_encode($response['erroredSkus']), 'error_count' => $response['error_count']];
                    } else {
...
//                                $error[] =  'Error : '.$sku['option_sku'].$feed_data['error'][0]['info'];
                                ...
                                $notUploadCount++;
                                $error[] = 'Status Not Updated for variant sku : ' . $sku['option_sku'] . ' of product sku : ' . $result['sku'];
                            }
                        }
            ...
            //}
            SiteController::refreshcache(MERCHANT_ID);
        }
        return json_encode($returnArr);
...
//            $jet_configuration = Data::sqlRecords('SELECT * FROM `jet_configuration` WHERE `merchant_id`=14', 'one');
    ...
    public function actionSyncproductstore()
    {
        ...
        /*if($countProducts > 10000 && $merchant_id != 1033)
        {
            ...
            if ($products && is_array($products)) {
                foreach ($products as $value) {
                    ...
                    if (is_array($response)) {
                        $jProduct += $response['count'];
                    } else {
                        $jProduct += $response;
                    }
                    if (isset($response['error_message'])) {
                        $returnArr = ['error' => true, 'message' => $response['error_message']];
                    }
                }
    ...
    public function actionUpdatewalmartinventory()
    {
        ...
        if ($count) {
            $response = $this->inventoryMgmtHelper->updateWalmartinventory($product);
            ...
            if (isset($response['errors'])) {
                ...
                if ($product['type'] == 'simple') {
                    $query = "UPDATE `walmart_product` SET `product_qty` = '" . $product['qty'] . "' WHERE `merchant_id`= '" . MERCHANT_ID . "' AND `product_id`='" . $product['id'] . "' ";
                } else {
                    $query = "UPDATE `walmart_product_variants` SET `option_qtys` = '" . $product['qty'] . "' WHERE `merchant_id`= '" . MERCHANT_ID . "' AND `product_id`='" . $product['id'] . "' AND `option_id`='" . $product['option_id'] . "'";
                }
                Data::sqlRecords($query, null, 'update');
                $returnArr = ['success' => true, 'count' => $count];
            }
    ...
    public function actionUpdateproductcount()
    {
        $merchant_id = Yii::$app->user->identity->id;
        ...
        $session->set('variant_count', $countProducts);

        return $this->render('variantcount', [
            'totalcount' => $countProducts,
    ...
    public function actionCountvariants()
    {
        $session = Yii::$app->session;
        $index = Yii::$app->request->post('index');
        //$products = isset($session['totalcount'][$index]) ? $session['totalcount'][$index] : [];
        $published_status = $session['published_status'];
        //$count = count($products);
        $limit = 250;
        $prod_count = 0;
        $returnArr = [];
        $index += 1; // by default page is one
        $sc = new ShopifyClientHelper(SHOP, TOKEN, WALMART_APP_KEY, WALMART_APP_SECRET);
        $products = $sc->call('GET', '/admin/products.json', ['fields' => "id, title, variants, product_type", 'published_status' => $published_status, 'limit' => $limit, 'page' => $index]);

        if (!isset($products['error'])) {

            foreach ($products as $prod) {
                foreach ($prod['variants'] as $variants) {
                    $prod_count++;
                }
            }
        }
        $returnArr = ['success' => true, 'success_count' => $prod_count];

        return json_encode($returnArr);
    }

    public function actionSavevariantcount()
    {
        $merchant_id = Yii::$app->user->identity->id;
        $returnArr = [];
        $variant_count = Yii::$app->request->post('variant_count');
        if(!empty($variant_count) && is_numeric($variant_count) && $variant_count > 0)
        {
            Data::sqlRecords("UPDATE `insert_product` SET `total_variant_products` = '".$variant_count."' WHERE `merchant_id`='".$merchant_id."'",null,'update');

            $key = $merchant_id.'total__variants_products';
            //\Yii::$app->cache->set($key, $prod_count, 86400); // time in seconds to store cache
            \Yii::$app->cache->set($key, $variant_count); // time in seconds to store cache

            $returnArr = ['success' => true, 'messsage' => 'Successfully updated '];
        }else{
            $returnArr = ['error' => true, 'messsage' => 'Updation Failed'];
        }
