 ...
 */
class WalmarttestorderController extends WalmartmainController
{
    ...
    public function actionIndex()
    {
        ...
        $resultdata = array();
        $queryObj = "";
        $query = "SELECT `shopify_order_id` FROM `walmart_order_details` WHERE merchant_id='" . $merchant_id . "' AND status='acknowledged' AND shopify_order_id=''";
    ...
    public function actionCancelOrder($config = false, $data = false)
    {
    ...
    public function actionCreate($config = false, $test = false)
    {
        ...
        if (defined('CONSUMER_CHANNEL_TYPE_ID') || $config)//foreach($model as $k=>$jetConfig)
        {
            try {
                ...
                $orderdata = $this->walmartHelper->getOrders(['status' => 'Acknowledged', 'createdStartDate' => $prev_date], Walmartapi::GET_ORDERS_SUB_URL, $test);
                ...
                $response = $orderdata;
                if (isset($response['errors'])) {
                    if (isset($response['errors']['error']['info']) && strpos($response['errors']['error']['info'], 'Requested content could not be found') !== false) {
                ...
                if (count($orders) > 0) {
                    ...
                    foreach ($orders as $order) {
                        ...
                        if (!$resultdata) {
                            ...
                            if (true || isset($order_ack['order_items']) && count($order_ack['order_items']) > 0) {
                                $skus = implode(',', $skus);
                                $ackData = array();
                                $ackResponse = $order;
    ...
    public function actionSyncorder($config = false)
    {
        ...
        $merchant_id = $config ? $config['merchant_id'] : Yii::$app->user->identity->id;
        if ($merchant_id == '359' || $merchant_id == '50' || $merchant_id == '95')
            return;
        $connection = Yii::$app->getDb();
        $model = "";
        $queryObj = "";
        ...
        if (defined('CONSUMER_CHANNEL_TYPE_ID') || $config) {
            try {
                ...
                $resultdata = array();
                $queryObj = "";
                $query = "SELECT `purchase_order_id`,`order_data` FROM `walmart_order_details` WHERE merchant_id='" . $merchant_id . "' AND status='acknowledged' AND shopify_order_id=0";
...
//                                                  }
    ...
    */
        ...
        if (!empty($orderAckCollection) && is_array($orderAckCollection)) {
            foreach ($orderAckCollection as $key => $value) {
                ...
                if (is_array($shipmentResponse) && isset($shipmentResponse['fulfillments']) && count($shipmentResponse['fulfillments']) > 0) {
                    ...
                    $shipdata = $orderdata1['elements']['order'][0]['orderLines'];
    ...
    {
        $query = "select `walmart_order_details`.`id`,`walmart_order_details`.`merchant_id`,`purchase_order_id`,`shopify_order_id`,`username`,`auth_key`,`consumer_id`,`secret_key`,`consumer_channel_type_id` FROM `walmart_order_details` inner join `user` ON `walmart_order_details`.`merchant_id`=`user`.`id`inner join `walmart_configuration` ON `walmart_order_details`.`merchant_id`=`walmart_configuration`.`merchant_id` WHERE `walmart_order_details`.`status`='inprogress' AND `shopify_order_id`!='' AND `walmart_order_details`.`merchant_id`!=14";
        $orderAckCollection = Data::sqlRecords($query, "all", "select");
        $count = 0;
    ...
    public function actionShipackorder()
    {
        $query = "SELECT `walmart_configuration`.`merchant_id`,`username`,`auth_key`,`consumer_id`,`secret_key`,`consumer_channel_type_id` FROM `walmart_configuration` INNER JOIN  `user` ON `walmart_configuration`.`merchant_id`=`user`.`id`";
        $orderAckCollection = Data::sqlRecords($query, "all", "select");
        $count = 0;
        foreach ($orderAckCollection as $key => $value) {
            $config = array();
            ...
            $config['secret_key'] = $value['secret_key'];
            $config['consumer_channel_type_id'] = $value['consumer_channel_type_id'];
            $config['token'] = $value['auth_key'];
            $config['shop_url'] = $value['username'];
            //print_r($data);die;
            $fetch = $this->actionCreate($config);
            $sync = $this->actionSyncorder($config);
            echo 'merchant_id: ' . $value['merchant_id'] . "<br><br>";
            $count++;
    ...
    public function actionTestupc()
    {
        ...
        /*if ($value) {
            if ($product['identifier_type'] == "UPC" && $upc_length < 12) {
            ...
            } elseif ($product['identifier_type'] == 'GTIN' && $upc_length < 14) {
                $zero_length = 14 - $upc_length;
    ...
    public function actionUpdatebarcodetype()
    {
        // die('dsfsdf');
        $data = Data::sqlRecords("SELECT * FROM `jet_product` WHERE `barcode_type` REGEXP '^[0-9]+$' ", 'all');


        if (!empty($data)) {
            foreach ($data as $value) {

                switch (strlen($value['upc'])) {
                    case 10:
                        $value['barcode_type'] = "ISBN";
                        break;
                    case 12:
                        $value['barcode_type'] = "UPC";
                        break;
                    case 13:
                        $value['barcode_type'] = "ISBN";
                        break;
                    case 14:
                        $value['barcode_type'] = "GTIN";
                        break;
                    default:
                ...
                $sql = "UPDATE `jet_product` SET `barcode_type`='" . $value['barcode_type'] . "' WHERE `id`='" . $value['id'] . "' ";

                Data::sqlRecords($sql, null, "update");

                Data::sqlRecords("UPDATE `jet_product` SET `barcode_type` = '" . $value['barcode_type'] . "' WHERE `id`= '" . $value['id'] . "' ", 'update');
                die('sdfdf');
            }
        }
    }

    public function actionCheckcustomprice()
    {
        $merchant_id = MERCHANT_ID;
        $price = 20;
        $price = Data::getCustomPrice($price, $merchant_id);
        print_r($price);
        die;
    }

    public function actionGetwalmartproductreport()
    {
      //  $reprice = new WalmartRepricing(API_USER, API_PASSWORD, CONSUMER_CHANNEL_TYPE_ID)
       $reprice = new WalmartRepricing('7a818a1e-b492-4c12-8719-3f883db6c6aa', '', '7b2c8dab-c79c-4cee-97fb-0ac399e17ade');
       $reprice->fetchWalmartProductReport();
       $reprice->fetchWalmartBuyboxReport();
       $reprice = new WalmartRepricing();
       $productData = $reprice->fetchWalmartProductReport();

       WalmartRepricing::fetchWalmartProductReport();
        $zip = new \ZipArchive();
        $res = $zip->open('/home/cedcoss/Downloads/ItemReport_10000003226_2017-03-24T084720.2680000.zip');
        if ($res === TRUE) {
            $zip->extractTo('/home/cedcoss/Downloads');
            $zip->close();
            echo 'woot!';
        } else {
            echo 'doh!';
        }
        $csv = array_map('str_getcsv', file('/home/cedcoss/Downloads/ItemReport_10000003226_2017-03-24T084720.2680000.csv'));
...
//        print_r($reprice);
    ...
    {
        $jet_product_variants = Data::sqlRecords("SELECT * FROM `jet_product_variants WHERE `merchant_id`='".MERCHANT_ID."'");
        print_r($jet_product_variants);die;
    }
}