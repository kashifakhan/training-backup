...
class Jetproductinfo extends component
{
        ...
        */
        ...
        if(is_array($options) && count($options)>0)
        {
            ...
            foreach($options as $val)
            {
                ...
                if($sku=="")
                    continue;
                if (isset($carray['sku_err']))
                   if(in_array($sku, $carray['sku_err']))
                    continue;
                ...
                if($product->sku==$sku)   {
                    $isParent=true;
                    $responseOptions['isParent'] = $product->sku;    
                }            
                ...
                if($product->jet_attributes)
                {
                    ...
                    $attribute_opt=json_decode(stripslashes($val->jet_option_attributes),true);
                ...
                else
                {
                    ...
                    $alternateImageRes = [];
                    $alternateImageRes = self::getAdditionalImages($images,$val->option_image,$parentmainImage);
                    if (!empty($alternateImageRes))
                ...
                if(!empty($SKU_Array))
                {
                    ...
                    if(/*count($response)==0 &&  */($status_code==202 || $status_code==201 || $status_code==200 ))
                    {                    	
                    	unset($filenameOrig);
                        $price = [];
    ...
    public static function checkBeforeDataPrepare($product=[],$merchant_id="")
    {
        ...
        if($product && trim($product->type))
        {
            ...
            if($sku=='')
            {
                ...
                $carray['validated_error'][$product->sku][] = "Missing Product Sku,";
                $cflag++;
            }
            if(strlen($title)<5 || strlen($title)>500)
            {
                ...
                $carray['validated_error'][$product->sku][] = "Invalid Title (between 5 to 500 characters), ";
                $cflag++;
            }
            if($brand=='')
            {
                ...
                $carray['validated_error'][$product->sku][] = "Missing brand,";
                $cflag++;
            }
            if($nodeid==''){
                $Errors['node_id_error']="Missing Jet Browse Node,";
                $Errors['node_id']=$product->sku;
                $carray['validated_error'][$product->sku][] = "Missing Jet Browse Node,";
                $cflag++;
            }
            if($image=='' || $ImageFlag){
                $Errors['image_error']="Missing or Invalid Image,";
                $Errors['image']=$product->sku;
                $carray['validated_error'][$product->sku][] = "Missing or Invalid Image,";
                $cflag++;
            }
            if(trim($product->type)=="simple")
            {
                ...
                if($upc=='' && $asin=='' && $mpn=='')
                {
                    $Errors['upc_error']="Missing unique identifier, ";
                    $Errors['upc']=$product->sku;
                    $cflag++;
                     $carray['validated_error'][$product->sku][] = "Missing unique identifier, ";
                }
                if(($price<=0 || ($price && !is_numeric($price))) || trim($price)=="")
                {
                    $Errors['price_error']="Invalid Price,";
                    $Errors['price']=$product->sku;
                    $carray['validated_error'][$product->sku][] ="Invalid Price, ";
                    $cflag++;
                }
                $type=self::checkUpcType($upc);
                if($type!="")
                ...
                if(!$isExistId)
                {
                    $Errors['asin_error_info']="Invalid Barcode or ASIN or MPN, ";
                    $Errors['upc']=$product->sku;
                    $carray['validated_error'][$product->sku][] = "Invalid Barcode or ASIN or MPN, ";
                    $cflag++;
                }
            }
            elseif(trim($product->type)=="variants")
            {
                ...
                $sqlNew = "SELECT `option_id`,`option_sku`,`option_qty`,COALESCE(`update_option_price`,`option_price`) as `option_price`,`option_unique_id`,`asin`,`option_mpn` FROM `jet_product_variants` WHERE `merchant_id`='".$merchant_id."' AND  `product_id`='".$product->id."' AND `upload_on_jet`='yes'";
                ...
                if(is_array($options) && count($options)>0)
                {
                    ...
                    {   
                        ...
                        $upc = (int)trim($pro['option_unique_id']);
                        $asin = trim($pro['asin']);
                        $mpn = trim($pro['option_mpn']);
                        if($opt_sku==""){
                            $Errors['sku_error_var'][]=$opt_sku;
                            $Errors['sku']=$product->sku;
                            $carray['sku_err'][]=$opt_sku;
                            $carray['validated_error'][$opt_sku][] = "Missing Product Sku, ";
                            $cflag++;
                        }
                        if($upc=='' && $asin=='' && $mpn==''){
                            ...
                            $Errors['upc']=$product->sku;
                            $carray['sku_err'][]=$opt_sku;
                            $carray['validated_error'][$opt_sku][] = "Missing unique identifier, ";
                            $cflag++;
                        }
                        if(trim($price) && !is_numeric($price)){
                            ...
                            $Errors['price']=$product->sku;
                            $carray['sku_err'][]=$opt_sku;
                            $carray['validated_error'][$opt_sku][] =  "Invalid Price, ";
                            $cflag++;
                        }
                        ...
                        if(!$isExistId)
                        {
                            ...
                            $Errors['upc']=$product->sku;
                            $carray['sku_err'][]=$opt_sku;
                            $carray['validated_error'][$opt_sku][] =   "Invalid/Missing Barcode or ASIN or MPN, ";
                            $cflag++;
                        }
                        ...
                        if($attr_ids !=""){
                            $attr_ids_arr=json_decode(stripslashes($attr_ids),true);
                        }
                        if(trim($jet_mapped) !=""){
                            $jet_mapped_arr=json_decode(stripslashes($jet_mapped),true);
                        }
                        $acflag=0;

                        if(is_array($attr_ids_arr) && count($attr_ids_arr)>0)
                        {
                            if(is_array($jet_mapped_arr) && !empty($jet_mapped_arr)>0)
                            {
                            ...
                            if($attrNotMapped)
                            {
                                ...
                                else
                                {
                                    ...
                                    $carray['sku_err'][]=$opt_sku;
                                    $carray['validated_error'][$opt_sku][] =  "Attribute not mapped, ";
                                    $cflag++;
                                }
                            }
                        }
        ...
        $carray['error']=$Errors;
        /*its for skip varaint*/
        if(trim($product->type)=="variants"){
           if(!empty($carray['sku_err'])){
            //if (!in_array($product->sku, $carray['sku_err']))
               $carray['success']=true;
    ...
    public static function productUpdateData($result,$data,$jetHelper,$fullfillmentnodeid,$merchant_id,$handle,$customPrice="",$newCustomPrice="", $onWalmart=false,$onNewEgg=false,$import_status=false)
    {
        try
        {
            ...
            if (mb_detect_encoding($new_description) !== 'UTF-8') {
                $new_description = utf8_encode($new_description);
            }           
            ...
            if(isset($data['options']) && count($data['options'])>0)
            {
                ...
                $attrValue=array();
                $attFlag=false;
                $attrValue=json_decode(stripslashes($result['attr_ids']),true);
        ...
        {
            self::getInstance()->createExceptionLog('ProductDataUpdate',$e->getMessage(),$merchant_id);
            exit(0);
        }
        catch(Exception $e)
        {
            $this->createExceptionLog('ProductDataUpdate',$e->getMessage());
            exit(0);
        }
    }
    ...
    public static function insertImportErrorProduct($id,$title,$type,$merchant_id)
    {
        ...
        {
            $query="INSERT INTO `product_import_error`(`id`, `merchant_id`, `missing_value`, `title`,`sku`) VALUES ('".$id."','".$merchant_id."','".$type."','".addslashes($title)."','".addslashes($sku)."')";
    ...
    public static function updateSkudataOnJet($product_id,$option_id,$changeData,$type,$jetHelper,$merchant_id,$variant_as_parent)
    {
        ...
        $SKU_Array = $unique = $Attribute_arr = $Attribute_array = [];
        $isUploadDes = $isUploadTitle = $isUploadVendor = $isUploadWeight = $isUploadbarcode = $isUploadVariant1 = $isUploadVariant2 = $isUploadVariant3 = $isUpload = false;

        $SKU_Array['product_title']=$response['product_title'];
        $SKU_Array['jet_browse_node_id']=$response['jet_browse_node_id'];
        $SKU_Array['multipack_quantity']=(int)$response['multipack_quantity'];
    ...
    public static function updatePriceOnJet($sku,$price,$jetHelper,$fullfillmentnodeid,$merchant_id,&$batchResponse=false)
    {
        try
        {
            ...
            $message.=json_encode($priceArray).PHP_EOL;
    ...
    public static function addVariant($product_id,$variantIds,$data,$images,$merchant_id,$changeType=false,$onWalmart=false,$onNewEgg=false)
    {
        ...
        {
            self::getInstance()->createExceptionLog('addVariant',$e->getMessage(),$merchant_id);
            exit(0);
        }
        catch(Exception $e)
        {
    ...
    public static function addNewVariants($data,$product_id,$merchant_id)
    {
        ...
        $shopName = Data::sqlRecords("SELECT * FROM `user` WHERE `id`='".$merchant_id."'",'one');
        $data['shopName'] = $shopName['shop_url'];
    ...
    public static function saveNewRecords($data, $merchant_id, $import_status = false, $connection = false, $jetRegistartion = false)
    {
        try
        {
            ...
            if(isset($data['id']))
            {
                ...
                /*$customData = JetProductInfo::getConfigSettings($merchant_id);
                $import_status = (isset($customData['import_status']) && $customData['import_status'])?$customData['import_status']:"";*/
                if ( $import_status=="published" &&  (is_null($data['published_at']) || trim($data['published_at'])=="" ) )
                {
                    self::insertImportErrorProduct($data['id'],$data['title'],'hidden_product',$merchant_id,$data['variants'][0]['sku']);
                ...
                if(is_array($variantData) && count($variantData)>0)
                {
                    ...
                    if(isset($data['product_type']) && $data['product_type'] !="")
                    {
                        ...
                        $query="";
                        $queryObj="";
                        $query='SELECT `category_id`,`product_type` FROM `jet_category_map` where merchant_id="'.$merchant_id.'" AND product_type="'.addslashes(trim($data['product_type'])).'" LIMIT 0,1';
                        $modelmap = Data::sqlRecords($query,"one","select");
                        if(!empty($modelmap) && isset($modelmap['category_id'],$modelmap['product_type']) )
                        {
                            $updateResult="";
                        ...
                        {
                            $queryObj="";
                            $query='INSERT INTO `jet_category_map`(`merchant_id`,`product_type`)VALUES("'.$merchant_id.'","'.addslashes(trim($data['product_type'])).'")';
                            Data::sqlRecords($query,null,'insert');
                        }
                    }
        ...
        {
            self::getInstance()->createExceptionLog('actionCurlproductcreate',$e->getMessage(),$merchant_id);
            return $e->getMessage();
            exit(0);
        }
    }
    ...
    public static function checkUpcSimple($product_upc="",$product_id="",$merchant_id="")
    {
        ...
        $query="SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  upc='".$product_upc."' AND id <> '".$product_id."' ";
        $main_products = Data::sqlRecords($query,'one','select');
        
        $query1 = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_unique_id='".$product_upc."' ";
        $variant = Data::sqlRecords($query1,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
        return false;
    }
    public static function checkUpcVariantSimple($product_upc="",$product_id="",$product_sku="",$merchant_id="")
    {
        ...
        $queryObj = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_sku <> '".addslashes($product_sku)."' AND option_unique_id='".$product_upc."' ";
        $variant = Data::sqlRecords($queryObj,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
        return false;
    ...
    public static function checkUpcVariants($product_upc="",$product_id="",$variant_id="",$variant_as_parent=0,$merchant_id="")
    {
        ...
        if($variant_as_parent){
            $queryObj = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  upc='".trim($product_upc)."' AND id <> '".$product_id."' ";            
        }else{
            $queryObj = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  upc='".trim($product_upc)."' ";            
        }
        $main_products = Data::sqlRecords($queryObj,'one','select');
        $queryObj1 = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_unique_id='".trim($product_upc)."' AND option_id <>'".$variant_id."' ";
        $variant = Data::sqlRecords($queryObj1,'one','select');
    ...
    public static function checkAsinSimple($product_asin="",$product_id="",$merchant_id="")
    {
        ...
        $queryObj = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  ASIN='".$product_asin."' AND id <> '".$product_id."' ";
        $main_products = Data::sqlRecords($queryObj,'one','select');
        
        $queryObj = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  asin='".$product_asin."' ";
        $variant = Data::sqlRecords($queryObj,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
    ...
    public static function checkAsinVariantSimple($product_asin="",$product_id="",$product_sku="",$merchant_id="")
    {
        ...
        $queryObj = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_sku <> '".addslashes($product_sku)."' AND asin='".$product_asin."' ";
        $variant = Data::sqlRecords($queryObj,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
    ...
    public static function checkAsinVariants($product_asin="",$product_id="",$variant_id="",$variant_as_parent=0,$merchant_id="")
    {
        ...
        $main_products=$variant=[];
        if($variant_as_parent){
            $queryObj = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  ASIN='".trim($product_asin)."' AND id <> '".$product_id."'";
        }else{
            $queryObj = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  ASIN='".trim($product_asin)."' ";
        }
        $main_products = Data::sqlRecords($queryObj,'one','select');

        $queryObj = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  asin='".trim($product_asin)."' AND option_id <> '".$variant_id."' ";
        $variant = Data::sqlRecords($queryObj,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
    ...
    public static function checkMpnSimple($product_mpn="",$product_id="",$merchant_id="")
    {
        ...
        $sql = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  mpn='".addslashes($product_mpn)."' AND id <> '".$product_id."' ";
        $main_products = Data::sqlRecords($sql,'one','select');
        
        $sql1 = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_mpn='".addslashes($product_mpn)."' ";
        $variant = Data::sqlRecords($sql1,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
        return false;
    ...
    public static function checkMpnVariantSimple($product_mpn="",$product_id="",$product_sku="",$merchant_id="")
    {
        ...
        $main_products = $variant = [];
        $variant_count=0;
        $sql1 = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  mpn='".addslashes($product_mpn)."' AND id <> '".$product_id."' ";
        $main_products = Data::sqlRecords($sql1,'one','select');
        
        $sql = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND  option_sku <> '".$product_sku."' AND option_mpn='".addslashes($product_mpn)."' ";
        $variant = Data::sqlRecords($sql,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
        return false;
    ...
    public static function checkMpnVariants($product_mpn="",$product_id="",$variant_id="",$variant_as_parent=0,$merchant_id="")
    {
        ...
        $main_products = $variant = [];
        if($variant_as_parent){
            $sql1 = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  mpn='".trim($product_mpn)."' AND id <> '".$product_id."' ";
            $main_products = Data::sqlRecords($sql1,'one','select');
        }else{
            $sql = "SELECT count(`id`) idcount FROM `jet_product` WHERE merchant_id='".$merchant_id."' AND  mpn='".trim($product_mpn)."' ";
            $main_products = Data::sqlRecords($sql,'one','select');
        }
        $main_product_count=count($main_products);
        //unset($main_products);
        $queryObj = "SELECT count(`option_id`) optionCount FROM `jet_product_variants` WHERE merchant_id='".$merchant_id."' AND option_mpn='".trim($product_mpn)."' and option_id <> '".$variant_id."' ";
        $variant = Data::sqlRecords($queryObj,'one','select');
        if($variant['optionCount'] > 0 || $main_products['idcount'] > 0){
            return true;
        }
    ...
    public static function updateDetails($value=[],$sync=[],$merchant_id,$webhook=false)
    {
        try
        {
            ...
            $result = Data::sqlRecords("SELECT title,sku,type,product_type,description,variant_id,image,qty,price,weight,attr_ids,vendor,upc,fulfillment_node,status FROM `jet_product` WHERE id='".$product_id."' LIMIT 0,1", "one", "select");
            $resultDetails=Data::sqlRecords("SELECT update_title,update_description,update_price FROM `jet_product_details` WHERE product_id='".$product_id."' LIMIT 0,1", "one", "select");
            if (!$result)
            {
                $customData = JetProductInfo::getConfigSettings($merchant_id);
                $import_status = (isset($customData['import_status']) && $customData['import_status'])?$customData['import_status']:"";
                Jetproductinfo::saveNewRecords($value,$merchant_id,$import_status);

                $shopName = Data::sqlRecords("SELECT * FROM `user` WHERE `id`='".$merchant_id."'",'one');
                $value['shopName'] = $shopName['username'];
                //send walmart url
                $url = Yii::getAlias('@webwalmarturl')."/walmart-webhook/productcreate?maintenanceprocess=1";
                Data::sendCurlRequest($value,$url);
                $count++;
                return $count;
            }
            ...
            if($product_type!=$result['product_type'] && isset($sync['sync-fields']['product_type']))
            {
            ...
            if (is_array($variants))
            {
                ...
                {
                    $updateProduct = $updateProductDetails = $updateProductVariant = "";
                    if(!self::validateSku($variant['sku'],$product_id,$merchant_id) || in_array($variant['sku'], $skus))
                    {
                        continue;
                    }
                    $skus[] = $variant['sku'];
                    ...
                    $option_barcode = Jetproductinfo::validateBarcode($variant['barcode'])?$variant['barcode']:"";
                    $variant_option1=isset($variant['option1'])?addslashes($variant['option1']):'';
                    $variant_option2=isset($variant['option2'])?addslashes($variant['option2']):'';
                    $variant_option3=isset($variant['option3'])?addslashes($variant['option3']):'';

                    //save data in `jet_product_variants`
                    $resulVar = Data::sqlRecords("SELECT option_title,option_sku,option_image,option_qty,option_weight,option_price,update_option_price,option_unique_id,variant_option1,variant_option2,variant_option3 FROM `jet_product_variants` WHERE option_id='".$option_id."' LIMIT 0,1", "one", "select");
                    ...
                    if (!isset($resulVar['option_sku']))
                    {
                        ...
                        if($result['variant_id'] == $option_id)
                        {
                            ...
                            if($result['sku'] != $option_sku && isset($sync['sync-fields']['sku']))
                            {
                                ...
                                $updateProduct.= "`status`='".Data::NOT_UPLOADED."',";
                            ...
                            if($result['price'] != $option_price && isset($sync['sync-fields']['price']))
                            {
                                ...
                                if(isset($resultDetails['update_price']) && $resultDetails['update_price'])
                                    $updateProductDetails.= "`update_price`='".(float)$option_price."',";
                            }
                    ...
                    else
                    {
                        ...
                        if($resulVar['option_sku'] != $option_sku && isset($sync['sync-fields']['sku']))
                        {
                            ...
                            $updateProductVariant.= "`status`='".Data::NOT_UPLOADED."',";
                        }
                        ...
                        if($resulVar['option_price'] != $option_price && isset($sync['sync-fields']['price']))
                        {
                            ...
                            if($resulVar['update_option_price'])
                                $updateProductVariant.= "`update_option_price`='".(float)$option_price."',";
                    ...
                    if($isMainProduct)
                    {
                        ...
                        if($isTitleChanged)
                        {
                            ...
                            if(isset($resultDetails['update_title']) && $resultDetails['update_title'])
                                $updateProductDetails.= "`update_title`='".$title."',";
                        }
                        if($isDescriptionChanged)
                        {
                            $updateProduct.= "`description`='".$description."',";
                            if(isset($resultDetails['update_description']) && $resultDetails['update_description'])
                                $updateProductDetails.= "`update_description`='".$description."',";
                        }
                        if($isImageChanged)
                        {
                    ...
                    if($updateProductDetails)
                    {
                        ...
                        $query="UPDATE `jet_product_details` SET ".$updateProductDetails." WHERE product_id=".$product_id;
                        //echo $query."<hr>";
                        Data::sqlRecords($query);
                    }
    ...
    public static function getAdditionalImages($allImages=[],$optionImage='',$parentmainImage='')
    {
    	if(!empty($allImages) && is_array($allImages))
    	{
    		$i=1;
    		$SKU_Array['alternate_images'] = []; 
    		foreach($allImages as $key=>$value)
    		{
    			if(($value==$optionImage) || ($optionImage=='' && ($value==$parentmainImage) ) )
    				continue;
    				if($i>8)
    					break;
    					if($value!='' && self::checkRemoteFile($value)==true){    	
