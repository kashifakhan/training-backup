...
class Jetapi extends Component{
	...
	/*public function isEnabled(){
			...
			//echo Mage::getStoreConfig('jet_options/ced_jet/active'); die;
			return $flag;
	}	*/
	...
	*/
	...
	public function CPostRequest($method,$postFields,&$status=false){
		...
		$directory = \Yii::getAlias('@webroot').'/var/jetapi/'.date('d-m-Y');
        if (!file_exists($directory)){
            mkdir($directory,0775, true);
        }
        $handle=fopen($directory.'/post.log','a');	
        fwrite($handle,$url.PHP_EOL);
        fwrite($handle,$postFields.PHP_EOL);
		...
		$status = $http_code;
		fwrite($handle,'Request Status : '.$http_code.PHP_EOL.PHP_EOL);
		fclose($handle);
	...
	/* PUT Request on Jetcom
			*
			*/
	public function CPutRequest($method, $post_field,&$status=false)
	{
        ...
        $handle=fopen($directory.'/put.log','a');	
        fwrite($handle,$url.PHP_EOL);
        fwrite($handle,$post_field.PHP_EOL);
	...
	public function CGetRequest($method,&$status=false)
	{
        ...
        $handle=fopen($directory.'/get.log','a');	
        fwrite($handle,$url.PHP_EOL);
