...
class Jetcategorytree extends component
{
    ...
    public static function getCategorypath($node){
        $data = JetCategory::find()->where(['category_id'=>$node])->one();
        if (!empty($data['title'])) {
            $path = $data['title']." -> ".$data['parent_title']." -> ".$data['root_title'];
        }
        else{
            $path = "category not mapped on item level";
        }
        return $path;
