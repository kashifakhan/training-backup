...
class JetDataValidation extends component
{
	...
	{			
		...
		$query = "SELECT `main`.`id` , `main`.`sku`, `main`.`title`, `main`.`type`, `main`.`description`, `main`.`variant_id`, `main`.`upc`,`main`.`ASIN`, `main`.`mpn`, `sub`.`option_sku`, `sub`.`option_title`, `sub`.`option_id`, `sub`.`option_unique_id`, `sub`.`option_mpn`, `sub`.`asin` ".
			" FROM `jet_product` as `main` LEFT JOIN `jet_product_variants` as `sub` ON `main`.`id`=`sub`.`product_id` AND `main`.`merchant_id`=`sub`.`merchant_id` ".
			"WHERE `main`.`merchant_id` = {$merchantId} AND `fulfillment_node`!=0 AND ".
			"(".
				"(".
		...
		{
			$main_title = $main_sku = $main_upc = $main_mpn = $main_asin = $main_desc = $sub_sku = $sub_title = $sub_asin = $sub_upc = $sub_mpn = $sub_barcode = $main_barcode = 0;
			$subTitleIssueKeywords = $mainTitleIssueKeywords = $subSkuIssueKeywords = $mainSkuIssueKeywords = $subBarcodeIssueKeywords = $mainBarcodeIssueKeywords = $mainDescIssueKeywords = "";
