...
use frontend\modules\jet\components\Profiler;
...
class TestWebhooksController extends Controller
{
	...
	public function getProductForTest(){
		...
		$data=json_decode('{"id":"4839342278","title":"Jeans Product","body_html":"Jeans New Product Color Bright yadav sj","vendor":"Fashion Apparel ","product_type":"Towels","created_at":"2016-02-24T06:33:40-05:00","handle":"jeans-new-product-1","updated_at":"2016-11-30T09:31:36-05:00","published_at":"2016-02-24T06:31:00-05:00","published_scope":"global","tags":"","variants":[{"id":"21084448518","product_id":"4839342278","title":"26","price":"9865196.00","sku":"256","position":"1","grams":"907","inventory_policy":"deny","fulfillment_service":"manual","inventory_management":"shopify","option1":"26","created_at":"2016-06-08T10:15:26-04:00","updated_at":"2016-11-30T09:31:36-05:00","taxable":"1","barcode":"768189046188","inventory_quantity":"0","weight":"0.907","weight_unit":"kg","old_inventory_quantity":"0","requires_shipping":"1"}],"options":[{"id":"5954663686","product_id":"4839342278","name":"Size","position":"1","values":["26"]}],"images":[{"id":"9758546054","product_id":"4839342278","position":"1","created_at":"2016-02-25T04:31:37-05:00","updated_at":"2016-11-07T10:14:01-05:00","src":"https:\/\/cdn.shopify.com\/s\/files\/1\/1009\/2336\/products\/images2.jpeg?v=1478531641"},{"id":"9758553670","product_id":"4839342278","position":"2","created_at":"2016-02-25T04:31:57-05:00","updated_at":"2016-09-12T11:45:18-04:00","src":"https:\/\/cdn.shopify.com\/s\/files\/1\/1009\/2336\/products\/images1_15ba8c69-196c-4967-a5a9-07aa1c1b52ad.jpeg?v=1473695118"},{"id":"9758558150","product_id":"4839342278","position":"3","created_at":"2016-02-25T04:32:11-05:00","updated_at":"2016-09-12T11:45:18-04:00","src":"https:\/\/cdn.shopify.com\/s\/files\/1\/1009\/2336\/products\/images_e806776e-3f18-41e7-8e5f-cf6e3ca5805d.jpeg?v=1473695118"}],"image":{"id":"9758546054","product_id":"4839342278","position":"1","created_at":"2016-02-25T04:31:37-05:00","updated_at":"2016-11-07T10:14:01-05:00","src":"https:\/\/cdn.shopify.com\/s\/files\/1\/1009\/2336\/products\/images2.jpeg?v=1478531641"},"shopName":"ced-jet.myshopify.com"}',true);
		return $data;
	}
	...
	public function getAllWebHooks()
	{
		return  [ ['action'=>'shopifywebhook/productupdate','data'=>[]],
				  ...
				  ['action'=>'shopifywebhook/productdelete','data'=>['id'=>7953757958]],
				  ['action'=>'shopifywebhook/cancelled','data'=>[]],
	...
	public function callCurl($path,$data,$json=true,$returnTransfer = true){
		...
		// URL on which we have to post data
		$baseUrl = Yii::getAlias('@webjeturl').'/';
		$url =  $baseUrl.$path;
	...
	public function actionProcess(){
		$query="SELECT * FROM `jet_maintenance_log`  ORDER BY `id` ASC";
		$collection = Data::sqlRecords($query,"all","select");
		foreach($collection as $row){
			$headers = [];
			$serverData = $row['server_data'];
			$serverData = $serverData == '' ?[]:json_decode($serverData,true);
			$headers['HTTP_X_SHOPIFY_SHOP_DOMAIN'] = isset($serverData['HTTP_X_SHOPIFY_SHOP_DOMAIN'])?$serverData['HTTP_X_SHOPIFY_SHOP_DOMAIN']:'test';
			
			$baseUrl = Yii::getAlias('@webjeturl').'/';
			$url =  $baseUrl.$row['action'].'?maintenanceprocess=1';
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array(                                                                          
			    'Content-Type: application/json',                                                                                
			    'Content-Length: ' . strlen($data_string),
			    'X-SHOPIFY-SHOP-DOMAIN: '.$headers['HTTP_X_SHOPIFY_SHOP_DOMAIN']
			    )                                                                       
			);   
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response = curl_exec($ch);
			$query="DELETE FROM `jet_maintenance_log` WHERE `id`='{$row['id']}'";
			$collection = Data::sqlRecords($query,"all","delete");
		}
