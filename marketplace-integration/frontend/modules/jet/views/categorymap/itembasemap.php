            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                	<div class="category-map-index content-section">
					    ...
					    <!-- <div class="clear"></div>  -->
					    <form id="category_map" class="form new-section" method="post" action="<?php echo \yii\helpers\Url::toRoute(['categorymap/saveitemcategory']) ?>">
							...
							<table id="map_producttype" class="table table-striped table-bordered">
								<tr>
									...
									<th data-toggle="tooltip" title="Product type on shopify">SKU(shopify)</th>
								...
								foreach($model as $value){
									...
									<tr <?= $i==1?'id="intro_first_tr"':"";?>>
										<td><?php echo $i; ?></td>
										<td <?= $i==1?'id="intro_first_td_label" data-step="1" data-position="bottom" data-intro="This is Shopify Product Type."':"";?>><?php echo $value['sku']; ?></td>
										...
										<?php $category_path_str="";?>
							  			...
							  			<td class="cat_root" <?= $i==1?'id="intro_first_td_root" data-step="2" data-position="bottom" data-intro="Select Root Category From DropDown corresponding to Product Type."':"";?>>
											<select id="select_<?=$i?>" name="type[<?=trim($value['sku'])?>][]" style="width:auto" class="form-control root" onchange="selectChild(this,1,<?php echo "'".trim($category_path_str)."'";?>,<?php echo "'".trim(addslashes($value['sku']))."'";?>)">
												<option value="">Please Select Category</option>
												<?php 
												foreach($data as $val)
												{
													if($val->level==2){
														if($value['fulfillment_node'] == $val->category_id){?>	
