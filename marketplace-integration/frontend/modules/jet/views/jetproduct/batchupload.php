<?php 
	...
	$ipmort_url=Yii::$app->request->baseUrl.'/jetproduct/index';
	$url=\yii\helpers\Url::toRoute(['jetproduct/startbatchupload']);
	$backUrl = \yii\helpers\Url::to(Yii::$app->request->referrer);
	if($param=="Inventory")
	{
		$url=\yii\helpers\Url::toRoute(['jetproduct/start-batch-upload-inventory']);
	}
	elseif($param=="Price")
	{
		$url=\yii\helpers\Url::toRoute(['jetproduct/start-batch-upload-price']);
	}
	elseif($param=="Resend")
	{
		$url=\yii\helpers\Url::toRoute(['jetproduct/resendbatchupload']);
		$param = "Resend Variant";
	}
?>
<style type="text/css" >
		...
		<div class="panel panel-default form new-section">
				<div class="jet-pages-heading">
					<div class="title-need-help">
						<h1 class="Jet_Products_style">Product <?= $param; ?> Upload Status</h1>
					</div>
		...
		<div class="block-content panel-body shopify-api ">			
				<ul class="warning-div" style="margin-top: 18px">
					<li style="background-color:#Fff;">
						...
						Starting Product <?= $param; ?> Upload execution, please wait...
					</li>
					<li style="background-color:#FFD;">
						...
						Warning: Please do not close the window during Uploading <?= $param; ?> data
					</li>
				</ul>
				...
				<ul id="profileRows">
					...
					<li id="liFinished" style="display:none;background-color:#Fff;">
						<img src="<?php echo yii::$app->request->baseUrl.'/frontend/images/batchupload/note_msg_icon.gif';?>" class="v-middle" style="margin-right:5px"/>
						Finished product <?= $param; ?> upload execution.
					</li>
				</ul>
			</div>
		</div>     
		...
		$.ajax({
        	...
        	success: function(json) {//transport
				...
				if(id < totalRecords){
					...
					var span = document.createElement('li');
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product(s) '+param+' Successfully Upload Requested.'+'</span>';
					span.style = 'background-color:#DDF';
			...
			error: function(){
				...
				if(id < totalRecords){
					...
					status_image.src = '<?php echo $succes_img ?>';
					var span = document.createElement('li');
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product(s) '+param+' Successfully Uploaded.'+'</span>';
					span.style = 'background-color:#DDF';
					my_id.parentNode.insertBefore(span, my_id);
					$(".warning-div").hide();
					$("#profileRows").css({'margin-top':'10px'});
