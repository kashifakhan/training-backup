...
$urlimage= yii\helpers\Url::toRoute(['jetproduct/saveimage']);
    ...
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content modal-content-error">
        <div class="modal-header">
          ...
          <h4 class="modal-title" style="text-align: center;font-family: "Comic Sans MS";">Update Product Images</h4>
        </div>
					...
					<table class="table table-striped table-bordered">
						...
						foreach ($collection as $key => $value) 
						{
							//$additional=[];
	                  		...
	                  		$html.='<tr><td>';	                  		
                  			if(count(explode(',',$value['image']))>0)
                  			{
                  				$images=[];
	    			        ...
	    			        <td>
	    			        	<form enctype="multipart/form-data" action="#" method="post" class="form-image" id="form-data-'.$value['id'].'">
									<div class="form-group" id="image_contrainer_'.$value['id'].'">
										...
										<input type="button" class="btn btn-primary" onclick=updateImage(this,"'.$value['id'].'") value="save & upload"/>
...
function updateImage(node,id){
	if($('#image_contrainer_'+id).has('li').length > 0){
		...
		alert("Please select new images to upload");
	}
}
</script>