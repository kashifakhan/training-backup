...
use yii\helpers\Html;
	...
	$ipmort_url=Yii::$app->request->baseUrl.'/updateproduct/bulk-shipping-exception';
	if(isset($param) && $param=="Bulk Shipping Exception")
	{
		$url=\yii\helpers\Url::toRoute(['updateproduct/mass-bulk-shipping-exception']);
	}
	else
	{
		$url=\yii\helpers\Url::toRoute(['productcsv/mass-archive-csv']);
	}
			...
			<div class="block-content panel-body shopify-api ">			
				...
				<ul id="profileRows">
					<li style="background-color:#DDF; ">
						...
						<?php echo 'Total '.$totalcount.' csv rows found.'; ?>
					</li>
	...
	function uploaddata(){
		...
		$.ajax({
        	...
        	success: function(json) {//transport
				...
				if(json.success)
				{
					current_postion=json.success;
					countOfSuccess+=json.bulk_shipping_exception;
					var span = document.createElement('li');
					span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span class="text">'+json.bulk_shipping_exception+' products '+param+' of page '+id+'...</span>';
					span.id = 'id-'+id;
