...
/* @var $model frontend\models\JetRegistration */
/* @var $form ActiveForm */

$model = new \frontend\modules\jet\models\JetRegistration();
...
<div class="site-register">
    <div class="content-section">
        <div class="">
            ...
            <?php $form = ActiveForm::begin(['id' => 'jetregistration-form', 'enableAjaxValidation' => true, 'enableClientValidation' => true]); ?>
            ...
            <!--  Already selling on jet (start) -->
            <?= $form->field($model, 'already_selling', ['options' => []])->dropDownList(['Yes' => 'Yes', 'No' => 'No'], ['prompt' => 'Choose...'])->label('Are you already selling on jet.com ?'); ?>
            <?= $form->field($model, 'is_uninstalled_previous', ['options' => ['style' => 'display:none;']])->dropDownList(['Yes' => 'Yes', 'No' => 'No'], ['prompt' => 'Choose...'])->label('Have you uninstalled the previous api provider app you were using?'); ?>

            <?= $form->field($model, 'previous_api_provider_name', ['options' => ['style' => 'display:none;'], 'inputOptions' => ['placeholder' => 'Enter previous api provider name (Optional)', 'class' => 'form-control']]) ?>

            <!--  Already selling on jet (end) -->
            ...
            <div class="required">
                ...
                <div class="terms-n-condition">
                    ...
                    <p>
                        CedCommerce, LLC currently provides Jet.com integration functionality with integration into
                        other
                        ...
                        You are responsible for keeping your billing information up to date and your account current. You 
						will not be liable for any sort of partial or prorated refund of your subscription fee for any time 
						during which you do not use the Services. The refund would not be applicable to the boundaries 
						beyond the operational field and the issues/concerns/liabilities not under the CedCommerce service 
						policies and norms. Sales of Products on Jet Marketplace entirely depends on your product , Jet  and
						Purchasing Client. App Provides System and Tools to make Sale Easier but does'nt guarantees sales.
						No Refund Policy is applicable in such scenarios where App is considered to generate revenue on 
						Jet.<br>
						In case any partial or complete refund is made to any users then his account would be completely 
						removed from our records. No support would be provided to the item that got published on Jet 
						Marketplace through our app thereafter as well as such product on Jet will be recalled/retired from 
						Jet Marketplace.<br><br>
                        ...
                        responsibility and you agree to not use CedCommerce in a way that violates Jet.com Terms of Use
                        (https://jet.com/terms-of-use) , itâ€™s API Terms of Use (https://developer.jet.com/tos) or
                        agreemenst with any other marketplace. By using CedCommerce.com, you agree to not violate any of
                        marketplace terms. In the event of violating any marketplace rules, we have the right to close
                        your account. We are also not responsible for any illegal use of CedCommerce.com by our users.<br><br>
            ...
            <?= Html::a('View & Download', Yii::$app->request->baseUrl . '/jet-policy/Jet Shopify Policy Document .pdf', ['class' => 'btn btn-primary', 'target' => '_blank']) ?>
...
<script type="text/javascript">
    ...
    var saveFlag = false;
    $('#jetregistration-form').on('submit', function (event) {
    ...
    $(".next").on('click', function () {
        $('#jetregistration-form').data('yiiActiveForm').submitting = true;
        var resp = $('#jetregistration-form').yiiActiveForm('validate');
    });
    ...
    $(document).ready(function () {
        ...
        $('#jetregistration-reference').change(function () {
            if ($(this).val() == 'Other') {
                $('#jetregistration-other_reference').css('display', 'block');
                $('.field-jetregistration-other_reference').css('display', 'block');
            }
            else {
                $('#jetregistration-other_reference').val("");
                $('#jetregistration-other_reference').css('display', 'none');
                $('.field-jetregistration-other_reference').css('display', 'none');
            }
        });

        $('#jetregistration-already_selling').change(function () {
            if ($(this).val() == 'Yes') {
                $('.field-jetregistration-is_uninstalled_previous').css('display', 'block');
                $('.field-jetregistration-previous_api_provider_name').css('display', 'block');
            }
            else {
                $('.field-jetregistration-is_uninstalled_previous').css('display', 'none');
                $('.field-jetregistration-previous_api_provider_name').css('display', 'none');
            }
        });

        $('input[name="JetRegistration[shipping_source][]"]').on('click', function () {
            var OtherFlag = false;
            $('input[name="JetRegistration[shipping_source][]"]').each(function () {
