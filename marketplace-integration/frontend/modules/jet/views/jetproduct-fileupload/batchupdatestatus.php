<?php 
	...
	$url = \yii\helpers\Url::toRoute(['jetproduct-fileupload/batchstatusupdate']);
		...
		<div class="panel panel-default form new-section">
				<div class="jet-pages-heading">
					<div class="title-need-help">
						<h1 class="Jet_Products_style">Product Bulk upload on Jet</h1>
					</div>
		...
		$.ajax({
        	...
        	success: function(json) 
        	{
				...
				if(json.errors){
					countOfSuccess++;
					var span = document.createElement('li');
					span.innerHTML = '<img class="v-middle" src="<?php echo $error_img ?>"><span class="text">'+json.errors+'</span>';
					span.id = 'id-'+id;
					span.style = 'background-color:#FDD';
					update_row.parentNode.insertBefore(span, update_row);
				...
				if(id < pages){
					...
					var span = document.createElement('li');
					if(notSku && notSku>0){
						span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product(s) Successfully Status(s) updated</span>';
					}else{
						span.innerHTML = '<img class="v-middle" src="<?php echo $succes_img ?>"><span id="update_status" class="text">'+countOfSuccess+' Product File Successfully Created.'+'</span>';
					}
					span.style = 'background-color:#DDF';
