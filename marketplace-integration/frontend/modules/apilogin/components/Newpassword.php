...
class Newpassword extends Component
{
    ...
    public function getNewpassword($Output)
    {
        ...
        $data = Datahelper::sqlRecords("SELECT `forgot_password_key` FROM `app_login_check` WHERE shop_url='" . $shop_url . "'", 'one');

        if (isset($oldpassword) && !empty($oldpassword)) {
            if (strlen($new_password) >= 8) {
                if (!empty($data) && $data['forgot_password_key'] == $oldpassword) {
                    $query = "UPDATE `app_login_check` SET `forgot_password_key`= NULL,`password`='" . $new_password . "', `status`='complete' where shop_url='" . $shop_url . "' ";
