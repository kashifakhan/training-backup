...
<div class="jet-email-template-form">
    ...
    echo '<div class="form-group field-jetemailtemplate-copy_template_from required">
    	...
    	foreach (JetEmailTemplate::find()->all() as $key => $value) {
...
$(document).ready(function(){
    $('#jetemailtemplate-template_title').on('blur',function(){
        var Html = $('#jetemailtemplate-template_title').val();
        $('#jetemailtemplate-template_path').val('email/'+Html+'.html');
    });
...
$(document).ready(function(){
	setTimeout(function(){ 
		var Name = $('#jetemailtemplate-template_title').val();
        if(Name){
            $.ajax({
                type:'POST',
                url:'htmltemplate',
...
<style type="text/css">
    .field-jetemailtemplate-template_path {
        display:none;
    }
