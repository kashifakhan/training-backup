...
class AppsNotConfiguredController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        $sql = "SELECT `jed`.`merchant_id`,`jed`.`email`,`jed`.`purchase_status`,`jed`.`install_status`,`jed`.`expired_on`,`jed`.`shop_url` FROM `{$table}` `jed` LEFT JOIN `{$table1}` `jc` ON `jed`.`merchant_id` = `jc`.`merchant_id` WHERE `jed`.`install_status`='1' AND `jc`.`merchant_id` IS NULL ";
