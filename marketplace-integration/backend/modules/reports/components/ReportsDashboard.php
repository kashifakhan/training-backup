...
class ReportsDashboard extends Component
{
	 ...
	 */
	public function getFreeClientNoRevenueNoProductsClientsCount()
	{
		$query = "select count(*) as count from (SELECT count( `jod`.`merchant_id` ) AS `count`,count( `jp`.`merchant_id` ) AS `live_products` , `jed`.`merchant_id` FROM `jet_shop_details` AS `jed` LEFT JOIN `jet_product` `jp` ON `jed`.`merchant_id` = `jp`.`merchant_id` AND `jp`.`status`='Available for Purchase' LEFT JOIN `jet_order_detail` `jod` ON `jed`.`merchant_id` = `jod`.`merchant_id` AND `jod`.`status` = 'complete' WHERE `jed`.`purchase_status` = 'Not Purchase' AND `jed`.`install_status`= '1' GROUP BY `jed`.`merchant_id` HAVING count <1 AND live_products <1 ) FINAL ";
		$count = Data::sqlRecords($query,'one','select');
	 ...
	 */
	public function getClientConfigurationStatus()
	{		
		$query = "SELECT count(*) as count FROM `jet_shop_details` `jed` LEFT JOIN `jet_test_api` `jta` ON `jed`.`merchant_id` = `jta`.`merchant_id` WHERE `jed`.`install_status`='1' AND `jta`.`merchant_id` IS NULL";
		$count = Data::sqlRecords($query,'one','select');
		$data = [];
		$data['testNotConfigured'] = isset($count['count'])?$count['count']:0;

		$query = "SELECT count(*) as count FROM `jet_shop_details` `jed` LEFT JOIN `jet_configuration` `jc` ON `jed`.`merchant_id` = `jc`.`merchant_id` WHERE `jed`.`install_status`='1' AND `jc`.`merchant_id` IS NULL";
	 ...
	 */
	public function getNoRevenueWithLiveProductsClientsCount()
	{		
	 ...
	 *
	 */
	public function getFreeAppWithRevenue()
	{
		$query = "select count(*) as count from (SELECT count( `jod`.`merchant_id` ) AS `count`,count( `jp`.`merchant_id` ) AS `live_products` , `jed`.`merchant_id` FROM `jet_shop_details` AS `jed` LEFT JOIN `jet_product` `jp` ON `jed`.`merchant_id` = `jp`.`merchant_id` AND `jp`.`status`='Available for Purchase' LEFT JOIN `jet_order_detail` `jod` ON `jed`.`merchant_id` = `jod`.`merchant_id` AND `jod`.`status` = 'complete'  WHERE `jed`.`purchase_status` = 'Not Purchase' GROUP BY `jed`.`merchant_id` HAVING count >0 AND live_products > 0 ) FINAL ";
	 ...
	 *
	 */
	public function getFreeAppWithNoRevenue()
	{
	...
	{
		$query = "SELECT COUNT(merchant_id) as count FROM jet_shop_details";
	...
	public function header()
	{
		...
		$model = $active_marchants_model = "";
		$active_marchants = $P_avail = $P_under = $P_complete = 0;

        $paidWithNoRevnue = $freeWithNoRevnue = $noRevenueWithLiveProductsClientsCount = $ratingData = $freeAppsWithRevenue = $freeAppsWithNoRevenue = $totalMerchant = 0;
//		$paidWithNoRevnue = $this->getNoRevenueNoProductsClientsCount();
//		$freeWithNoRevnue = $this->getFreeClientNoRevenueNoProductsClientsCount();
//		$noRevenueWithLiveProductsClientsCount = $this->getNoRevenueWithLiveProductsClientsCount();
//		$ratingData = $this->getRatingData();
//		$freeAppsWithRevenue = $this->getFreeAppWithRevenue();
//		$freeAppsWithNoRevenue = $this->getFreeAppWithNoRevenue();
//		$totalMerchant = $this->getTotalMerchant();

        $clientConfigStatus =$count['count']=$uninstallCount['count'] = 0;

//		$clientConfigStatus = $this->getClientConfigurationStatus();
//		$count = Data::sqlRecords("SELECT count(*) as count from `jet_shop_details` where DATE(installed_on) = DATE(NOW())","one","select");
//		$uninstallCount = Data::sqlRecords("SELECT count(*) as count from `jet_shop_details` where DATE(uninstall_date) = DATE(NOW()) and install_status='0'","one","select");


		$detailArray['count']=$count['count'];
		$detailArray['uninstall_count']=$uninstallCount['count'];

//		$expiringToday = Data::sqlRecords("SELECT count(*) as count from `jet_shop_details` where (DATE(expired_on) BETWEEN  DATE(NOW()) and DATE(NOW())+INTERVAL 7 DAY) and install_status='1'",'one','select');
//		$expiringToday = $expiringToday['count'];
		$expiringToday = 0;
//		$expiringInThreeDays = Data::sqlRecords("SELECT count(*) as count from `jet_shop_details` where (DATE(expired_on) BETWEEN  DATE(NOW()) and DATE(NOW())+INTERVAL 3 DAY) and install_status='1'",'one','select');
